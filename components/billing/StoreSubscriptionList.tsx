...
export function StoreSubscriptionList({
  ...
  const [activatingStores, setActivatingStores] = useState<Set<string>>(
    new Set(),
  );
  ...
  const unsubscribedIds = unsubscribedStores
    .map((s) => s.store_id)
    .sort()
    .join(",");
  ...
  if (subscriptions.length === 0 && unsubscribedStores.length === 0)
    return null;
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {!isBillingOwner && (
                            <Badge
                              variant="outline"
                              className="bg-muted/50 text-muted-foreground border-muted text-[10px] px-1.5 py-0 shrink-0"
                            >
                              Co-Owner
                            </Badge>
                          )}
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          <Badge
                            variant="outline"
                            className={`${config.className} border text-[10px] px-1.5 py-0`}
                          >
                      ...
                      <div className="flex items-center gap-1.5 sm:shrink-0 ml-auto sm:ml-0">
                        {sub.cancel_at_period_end ? (
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-7 text-xs text-muted-foreground hover:text-emerald-400 hover:bg-emerald-500/10"
                            onClick={() =>
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium truncate">
                          {storeUser.store?.name || "Unknown Store"}
                        </p>
          ...
          {(() => {
            const billedSubs = subscriptions.filter(
              (s) =>
                ...
                billingOwnerStoreIds.has(s.store_id),
            );
            if (billedSubs.length === 0) return null;
            const monthlyPrice = getMonthlyPriceDisplay();
            const total =
              billedSubs.length * (BILLING_CONFIG.PRICE_AMOUNT_PENCE / 100);
            return (
              <div className="mt-3 pt-3 border-t flex items-center justify-between text-sm">
                <span className="text-muted-foreground">
                  {billedSubs.length} store{billedSubs.length !== 1 ? "s" : ""}{" "}
                  × {monthlyPrice}
                </span>
                <span className="font-semibold">£{total.toFixed(0)}/month</span>
              </div>
            );
          })()}
        </CardContent>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              billing period. After that, you&apos;ll lose access to this
              store&apos;s features.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="space-y-2 my-3">
            {[
              ...
              "Team member access and shift scheduling",
