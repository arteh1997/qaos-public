...
// Capture current time once at module load — close enough for trial-days calculation
// and avoids calling Date.now() during render (which the linter flags as impure).
const MODULE_LOAD_TIME = Date.now();

export function PlanOverviewCard({
  subscriptions,
  billingOwnerStoreIds,
          ...
          <div className="space-y-1.5">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {statusConfig && (
                <Badge
                  variant="outline"
                  className={`${statusConfig.className} border text-[11px]`}
                >
          ...
          {billedCount > 0 && (
            <div className="text-right text-sm text-muted-foreground">
              <p className="font-medium text-foreground">
                {billedCount} store{billedCount !== 1 ? "s" : ""} ×{" "}
                {monthlyPrice}
              </p>
          ...
          {isPastDue && (
            <div className="flex items-center gap-2 text-sm text-destructive">
              ...
              <span className="font-medium">
                Payment failed — please update your card below
              </span>
            </div>
          )}
          {isCancelling && cancelDate && (
            <div className="flex items-center gap-2 text-sm text-amber-400">
              ...
              <span>
                ...
                {format(new Date(cancelDate), "MMM d, yyyy")}
              </span>
            </div>
          )}
          {!isPastDue &&
