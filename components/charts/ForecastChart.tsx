...
export function ForecastChart({
  ...
  const chartData = useMemo(() => {
    ...
    >((acc, f) => {
      const prevQty =
        acc.length > 0 ? acc[acc.length - 1].runningQty : currentQuantity;
      const runningQty = Math.max(0, prevQty - f.predicted);
      return [
        ...acc,
        {
          date: new Date(f.date + "T12:00:00").toLocaleDateString("en-US", {
            month: "short",
            day: "numeric",
          }),
          ...
          projectedStock: Math.round(runningQty * 100) / 100,
          runningQty,
        },
      ];
    }, []);
...
export function StockProjectionChart({
  ...
  const chartData = useMemo(() => {
    const forecastPoints = forecast.reduce<
      { date: string; stock: number; runningQty: number }[]
    >((acc, f) => {
      const prevQty =
        acc.length > 0 ? acc[acc.length - 1].runningQty : currentQuantity;
      const runningQty = Math.max(0, prevQty - f.predicted);
      return [
        ...acc,
        {
          ...
          stock: Math.round(runningQty * 100) / 100,
          runningQty,
        },
      ];
    }, []);
    return [{ date: "Today", stock: currentQuantity }, ...forecastPoints];
  }, [forecast, currentQuantity]);
            ...
            >
              ...
              <Tooltip
                formatter={(value: unknown) => [
                  `${Number(value).toFixed(1)} ${unitOfMeasure}`,
                  "Stock",
                ]}
                labelClassName="font-semibold"
              />
