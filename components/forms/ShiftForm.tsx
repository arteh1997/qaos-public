...
export function ShiftForm({
  ...
  const getShiftPatternsForDate = (dateStr: string) => {
    if (!store?.weekly_hours || !dateStr) return { patterns: [], dayOfWeek: null, storeHours: null }
  ...
  const getShiftTimes = (entry: ShiftEntry): { start: string; end: string } => {
    if (entry.mode === 'preset' && entry.date) {
      ...
      if (shiftPatterns) {
        const pattern = shiftPatterns[entry.pattern as keyof typeof shiftPatterns]
  ...
  const handleSubmit = async () => {
    if (!selectedStore || !selectedStaff || !allShiftsHaveDates || shifts.length === 0) return
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            {shifts.map((entry, index) => {
              const { patterns, dayOfWeek, storeHours, shiftPatterns } = entry.date
                ? getShiftPatternsForDate(entry.date)
                : { patterns: [], dayOfWeek: null, storeHours: null, shiftPatterns: null }
              const hasPatterns = patterns.length > 0

              return (
                <div key={entry.id} className="border rounded-lg p-3 space-y-3 bg-muted/30">
                  ...
                  {entry.date && (
                    <>
                      ...
                      <div className="flex gap-2">
                        <Button
                          type="button"
                          variant={entry.mode === 'preset' ? 'default' : 'outline'}
                          size="sm"
                          onClick={() => updateShiftEntry(entry.id, { mode: 'preset' })}
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() => updateShiftEntry(entry.id, { mode: 'custom' })}
                          className="flex-1"
                        >
                          Custom Time
                        </Button>
                      ...
                      {entry.mode === 'preset' && hasPatterns && (
                        <div className={`grid gap-2 ${patterns.length === 3 ? 'grid-cols-3' : patterns.length === 2 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                          {patterns.map(pattern => {
                            const Icon = PATTERN_ICONS[pattern.id] || Clock
                            const isSelected = entry.pattern === pattern.id
                            const shiftSlot = shiftPatterns?.[pattern.id as keyof typeof shiftPatterns]
                            ...
                            return (
                              <button
                                key={pattern.id}
                                type="button"
                                onClick={() => updateShiftEntry(entry.id, { pattern: pattern.id })}
                            ...
                            <div className="flex items-center gap-2 p-2 bg-indigo-50 dark:bg-indigo-950/50 rounded text-xs text-indigo-700 dark:text-indigo-300">
                              ...
                              <span>
                                ...
                                <span className="font-medium">
                                  {format(addDays(parseISO(entry.date), 1), 'EEE, MMM d')}
                                </span>
                              </span>
                            </div>
                          )}
          ...
          {/* Summary */}
          {selectedStore && selectedStaff && allShiftsHaveDates && shifts.length > 0 && (
              ...
              <div className="text-muted-foreground mt-1 space-y-1">
                ...
                <div className="mt-2 space-y-1">
                  {shifts.map((entry, i) => {
                    ...
                    const overnight = isOvernightShift(times.start, times.end)
                    return (
                      <p key={entry.id} className="text-xs">
                        {dayOfWeek ? DAY_NAMES[dayOfWeek].slice(0, 3) : ''} {format(parseISO(entry.date), 'MMM d')}: {times.start} - {times.end}
                        {overnight && (
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button
              ...
              disabled={!selectedStore || !selectedStaff || !allShiftsHaveDates || shifts.length === 0 || isLoading}
            >
              {isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              {shift ? 'Update Shift' : `Create ${shifts.length} Shift${shifts.length > 1 ? 's' : ''}`}
            </Button>
          </div>
        </div>
      </DialogContent>
