...
interface InventoryItemFormBodyProps {
  item?: InventoryItem | null;
  onSubmit: (
    data: InventoryItemFormData,
    options?: { costPerUnit?: number },
...
function InventoryItemFormBody({
  ...
  const [costPerUnit, setCostPerUnit] = useState(
    currentCost && currentCost > 0 ? currentCost.toString() : "",
  );
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Cost per single unit</p>
        </div>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        {/* key forces the form to remount with fresh defaultValues when the dialog opens or item changes */}
        <InventoryItemFormBody
          key={`${open}-${item?.id ?? "new"}-${currentCost}`}
          item={item}
          onSubmit={onSubmit}
          isLoading={isLoading}
