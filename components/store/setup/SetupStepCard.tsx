  ...
  return (
    <Card
      className={cn(
        ...
        step.isComplete && "border-green-500/50 bg-green-500/10",
        isLocked && "opacity-50",
    ...
    >
      <CardContent className="p-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-1 sm:gap-2">
              ...
              {step.isComplete && (
                <Badge
                  variant="default"
                  className="text-[10px] sm:text-xs px-1.5 py-0 bg-green-500"
                >
                  Complete
                </Badge>
              )}
            ...
            <p className="text-xs sm:text-sm text-muted-foreground mt-0.5 line-clamp-2">
              {isLocked
                ? "Complete the required steps above first"
                : step.description}
            </p>
          </div>
        ...
        {isExpanded && !isLocked && children && (
          <div className="px-3 sm:px-4 pb-4 pt-2 border-t">{children}</div>
        )}
      </CardContent>
    </Card>
  );
