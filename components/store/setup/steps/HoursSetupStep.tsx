...
import { DAYS_OF_WEEK, DAY_LABELS, getDefaultWeeklyHours, calculateDefaultShiftPatterns } from '@/lib/validations/store'
...
export function HoursSetupStep({ store, isComplete, onComplete }: HoursSetupStepProps) {
  ...
  const savedHoursRef = useRef<WeeklyHours>(JSON.parse(JSON.stringify(weeklyHours)))
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const defaultOpenTime = firstOpenDay ? weeklyHours[firstOpenDay].opening_time : '09:00'
      const defaultCloseTime = firstOpenDay ? weeklyHours[firstOpenDay].closing_time : '22:00'
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Set your operating hours for each day. Toggle days off if you&apos;re closed, and expand shift times to configure staff shifts.
      </p>
