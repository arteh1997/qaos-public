...
function getShiftStatus(shift: Shift) {
  ...
  if (now < start) {
    ...
    if (clockIn)
      return { label: "Clocked In Early", variant: "default" as const };
  ...
  if (now <= end) {
    ...
    if (clockOut) {
      ...
      if (minutesBeforeEnd >= LEFT_EARLY_THRESHOLD)
        return { label: "Left Early", variant: "warning" as const };
      return { label: "Completed", variant: "success" as const };
    }
    const minutesLate = (clockIn.getTime() - start.getTime()) / 60000;
    if (minutesLate > LATE_THRESHOLD)
      return { label: "Active · Late", variant: "warning" as const };
  ...
  if (minutesBeforeEnd >= LEFT_EARLY_THRESHOLD)
    return { label: "Left Early", variant: "warning" as const };
  if (minutesLate > LATE_THRESHOLD)
    return { label: "Late Arrival", variant: "warning" as const };
  if (minutesAfterEnd > OVERTIME_THRESHOLD)
    return { label: "Overtime", variant: "secondary" as const };
  return { label: "Completed", variant: "success" as const };
}
...
function formatVariance(
  ...
  if (Math.abs(diffMin) <= 2)
    return { text: "On time", className: "text-emerald-400" };

  if (type === "in") {
    // Clock in: early = fine, late = bad
    if (diffMin < 0)
      return {
        text: `${Math.abs(diffMin)}m early`,
        className: "text-emerald-400",
      };
    return { text: `${diffMin}m late`, className: "text-amber-400" };
  }
  // Clock out: early = bad, overtime = neutral
  if (diffMin < 0)
    return { text: `${Math.abs(diffMin)}m early`, className: "text-amber-400" };
...
export function TimelineView({
  ...
  const dayShifts = useMemo(
    () =>
      shifts.filter((shift) =>
        isSameDay(new Date(shift.start_time), selectedDay),
      ),
    [shifts, selectedDay],
  );
                  ...
                  <div className="px-4 py-2.5 bg-muted/30 flex items-center justify-between">
                    ...
                    {entityShifts.length > 0 && (
                      <span className="text-xs text-muted-foreground shrink-0 ml-2">
                        {entityShifts.length} shift
                        {entityShifts.length !== 1 ? "s" : ""}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="px-4 py-2 space-y-1.5">
                    {entityShifts.length === 0 ? (
                      ...
                      entityShifts.map((shift) => {
                        const shiftStore = stores.find(
                          (s) => s.id === shift.store_id,
                        );
                        const shiftUser = staff.find(
                          (s) => s.id === shift.user_id,
                        );
                        ...
                        return (
                          <div
                            key={shift.id}
                            className={`rounded-md border ${color.border} px-3 py-2`}
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-1.5 min-w-0">
                                <span
                                  className={`h-2 w-2 rounded-full shrink-0 ${color.bar}`}
                                />
                                <span className="text-sm font-medium truncate">
                                  {isStore
                                    ? shiftUser?.full_name ||
                    ...
                    <div className="relative py-2" style={{ minHeight: 44 }}>
                      ...
                      <div className="absolute inset-0 flex pointer-events-none">
                        {hours.map((hour) => (
                          <div
                            key={hour}
                            className="flex-1 border-l border-dashed border-border/40 first:border-l-0"
                          />
                        ))}
                      </div>
