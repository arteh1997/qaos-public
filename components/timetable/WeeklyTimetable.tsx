...
export function WeeklyTimetable({
  ...
  const groupedShifts = useMemo(() => {
    ...
    const rows =
      viewMode === "staff"
        ? staff.filter(
            (s) => !selectedStoreId || s.store_id === selectedStoreId,
          )
  ...
  const rows =
    viewMode === "staff"
      ...
      : stores.filter(
          (s) => s.is_active && (!selectedStoreId || s.id === selectedStoreId),
        );
              ...
              <tr>
                ...
                {weekDates.map((date, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`font-medium ${today ? "text-primary" : ""}`}
                      >
                        {day}
                      </div>
                      <div
                        className={`text-xs ${today ? "text-primary" : "text-muted-foreground"}`}
                      >
                        {dateStr}
                      </div>
                    </th>
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  ...
                  >
                    No {viewMode === "staff" ? "staff members" : "stores"} to
                    display
                  </td>
                </tr>
              ) : (
                rows.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="border-b hover:bg-muted/30">
                      <td className="p-2 font-medium text-sm">{rowName}</td>
                      {weekDates.map((date, dayIndex) => {
                        const cellShifts =
                          groupedShifts[row.id]?.[dayIndex] || [];
                        const today = isToday(date);
                          ...
                          >
                            <div className="min-h-[50px] space-y-1">
                              {cellShifts.map((shift) => {
                                ...
                                const colorClass =
                                  SHIFT_COLORS[pattern] || SHIFT_COLORS.Custom;
