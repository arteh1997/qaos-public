...
import { Loader2, Clock, Sun, Sunset, Moon, AlertCircle, MoonStar } from 'lucide-react'
...
export function QuickShiftModal({
  ...
  const [selectedStaff, setSelectedStaff] = useState<string>(preselectedStaffId || '')
  const [customStartTime, setCustomStartTime] = useState('09:00')
  const [customEndTime, setCustomEndTime] = useState('17:00')

  // Reset selections when modal opens with new preselected values
  useEffect(() => {
    if (open) {
      setSelectedStore(preselectedStoreId || '')
      setSelectedStaff(preselectedStaffId || '')
    }
  }, [open, preselectedStoreId, preselectedStaffId])
  ...
  const hasHoursForDay = storeHoursForDay?.isOpen && storeHoursForDay?.openingTime && storeHoursForDay?.closingTime
  const hasShiftPatterns = storeShiftPatterns && (
    storeShiftPatterns.opening || storeShiftPatterns.mid || storeShiftPatterns.closing
  )
  ...
  const presetTimes = useMemo(() => {
    ...
    const shiftSlot = storeShiftPatterns[selectedPattern as keyof typeof storeShiftPatterns]
    ...
    return { start, end }
  }, [selectedPattern, storeShiftPatterns, hasShiftPatterns, date])
  ...
  const availablePatterns = useMemo(() => {
    ...
    return DEFAULT_SHIFT_PATTERNS.filter(pattern => {
      const slot = storeShiftPatterns[pattern.id as keyof typeof storeShiftPatterns]
      return slot?.start_time && slot?.end_time
    })
  }, [storeShiftPatterns])

  // Auto-select first available pattern when patterns change
  useEffect(() => {
    if (availablePatterns.length > 0 && !availablePatterns.find(p => p.id === selectedPattern)) {
      setSelectedPattern(availablePatterns[0].id)
    }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            >
              <SelectTrigger>
                <SelectValue placeholder={selectedStore ? 'Select staff' : 'Select store first'} />
              </SelectTrigger>
              <SelectContent>
            ...
            {mode === 'preset' && hasShiftPatterns && storeHoursForDay && (
              <>
                <p className="text-xs text-muted-foreground">
                  {DAY_NAMES[dayOfWeek]} hours: {storeHoursForDay.openingTime} - {storeHoursForDay.closingTime}
                </p>
                <div className={`grid gap-2 ${availablePatterns.length === 3 ? 'grid-cols-3' : availablePatterns.length === 2 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                  {availablePatterns.map(pattern => {
                    const Icon = PATTERN_ICONS[pattern.id] || Clock
                    const isSelected = selectedPattern === pattern.id
                    const shiftSlot = storeShiftPatterns?.[pattern.id as keyof typeof storeShiftPatterns]
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-[10px] text-muted-foreground mt-0.5">
                          {formatShiftTime(startDate)} - {formatShiftTime(endDate)}
                        </div>
                      </button>
                    )
                  })}
          ...
          {selectedStore && selectedStaff && (
            <div className="bg-muted/50 rounded-lg p-3 text-sm">
              ...
              <div className="text-muted-foreground mt-1">
                {availableStaff.find(s => s.id === selectedStaff)?.full_name || 'Staff'} at {store?.name}
                <br />
                {mode === 'preset' && presetTimes ? (
                  <>
                    {formatShiftTime(presetTimes.start)} - {formatShiftTime(presetTimes.end)}
                  </>
                ) : (
                  <>
                    {customStartTime} - {customEndTime}
                    {isOvernightShift && (
                      <span className="text-indigo-600 dark:text-indigo-400"> (+1 day)</span>
                    )}
                  </>
                )}
              </div>
