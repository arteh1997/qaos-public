...
import { usePosMenuItems, useCreatePosMapping, useDeletePosMapping } from '@/hooks/usePosProviders'
...
export function MenuSyncTable({ storeId, connectionId, connectionProvider }: MenuSyncTableProps) {
  const [search, setSearch] = useState('')
  const [mappingItemId, setMappingItemId] = useState<string | null>(null)
  const [inventorySearch, setInventorySearch] = useState('')
  const [quantities, setQuantities] = useState<Record<string, number>>({})

  const { data: menuItems, isLoading: menuLoading, refetch: refetchMenu } = usePosMenuItems(storeId, connectionId)
  const { data: mappings, isLoading: mappingsLoading } = usePosItemMappings(storeId, connectionId)
  ...
  const handleMapItem = async (posItemId: string, posItemName: string, inventoryItemId: string) => {
    try {
  ...
  const mappedCount = menuItems?.filter(i => mappingLookup.has(i.pos_item_id)).length || 0
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <CardTitle className="text-base">Map Menu Items to Inventory</CardTitle>
              <CardDescription>
        ...
        <CardContent className="space-y-3">
          ...
          {filteredItems.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">
                {search ? 'No items match your search' : 'No menu items found from your POS'}
              </p>
            </div>
          ) : (
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-medium truncate">{item.pos_item_name}</p>
                          {item.category && (
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {isMapped ? (
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() => {
                              setMappingItemId(isExpanded ? null : item.pos_item_id)
                              setInventorySearch('')
                            }}
                          >
                    ...
                    {isExpanded && !isMapped && (
                      <div className="mt-3 rounded-lg border border-border p-3 bg-muted/30 space-y-3">
                        ...
                        <div className="flex gap-3 items-end">
                          ...
                          <div className="w-24">
                            <label className="text-xs text-muted-foreground block mb-1">Qty/sale</label>
                            <Input
                              type="number"
                              min={0.01}
                              step={0.01}
                              ...
                              onChange={e => setQuantities(prev => ({
                                ...prev,
                                [item.pos_item_id]: Number(e.target.value) || 1,
                              }))}
                              className="text-sm"
                            />
                          </div>
                              ...
                              >
                                <div className="min-w-0">
                                  ...
                                  <p className="text-xs text-muted-foreground">
                                    {inv.inventory_item?.category || 'No category'}
                                    {' · '}
