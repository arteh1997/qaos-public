...
export function DateRangePicker({
  ...
  const [internalRange, setInternalRange] = React.useState<
    DateRange | undefined
  >(undefined);
  // State to track whether next click is start or end (used in render for footer display)
    ...
    for (const preset of presets) {
      ...
      if (
        ...
        format(range.from, "yyyy-MM-dd") ===
          format(rangeToCheck.from, "yyyy-MM-dd") &&
      ...
      <PopoverContent className="w-auto p-0" align={align}>
        ...
        {isMobile && (
          <div className="border-b p-2">
            <Select
              ...
              onValueChange={(val) =>
                handlePresetChange(val as DateRangePreset)
              }
            >
        ...
        {!isMobile && (
          <div className="border-t px-4 py-2 text-sm">
            {isSelectingEnd && internalRange?.from ? (
              <span className="text-primary font-medium">
                {format(internalRange.from, "MMM d, yyyy")} &rarr; Select end
                date
              </span>
