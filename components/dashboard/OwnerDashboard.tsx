...
function DashboardContent() {
  ...
  const { data: missingCounts, isLoading: missingLoading } =
    useMissingCounts(currentStoreId);
  ...
  useEffect(() => {
    async function fetchStoreInventory() {
      ...
      try {
        const { data, error } = await supabaseFetch<StoreInventory>(
          "store_inventory",
          {
  ...
  const inventoryMetrics = useMemo(() => {
    ...
    const outOfStock = storeInventory.filter(
  ...
  const busiestItems = useMemo(() => {
    ...
    return analytics.topMovingItems.slice(0, 5).map((item, idx) => ({
      rank: idx + 1,
      name: item.name,
  ...
  const lastDelivery = useMemo(() => {
    ...
    const deliveries = recentActivity.filter(
      (a) => a.action_type === "Reception",
    );
      ...
      {hasUrgentIssues && (
        <div className="space-y-3">
          ...
          <div className="grid gap-4 md:grid-cols-2">
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-base font-semibold text-destructive">
                      {inventoryMetrics.outOfStock} Item
                      {inventoryMetrics.outOfStock !== 1 ? "s" : ""} Out of
                      Stock
                    </CardTitle>
                  </div>
                </CardHeader>
            ...
            {isMissingCount && (
              <Card className="border-amber-500/40 bg-amber-500/10 flex flex-col">
                <CardHeader className="pb-3">
                  <div className="flex items-center gap-2">
                ...
                <CardContent className="flex flex-col flex-1">
                  <p className="text-sm text-amber-400 flex-1">
                    Today&apos;s stock count hasn&apos;t been completed yet.
                    Complete it to ensure accurate inventory tracking.
                  </p>
                  {canCount && (
            ...
            {haccpDueChecks.length > 0 && (
              <Card className="border-amber-500/40 bg-amber-500/10 flex flex-col">
                ...
                <CardContent className="flex flex-col flex-1">
                  <div className="text-sm text-amber-400 flex-1 space-y-1">
        ...
        <div className="grid gap-4 grid-cols-2 sm:grid-cols-3">
          ...
          <Link href="/inventory?category=all">
            <StatsCard
              ...
              description={
                inventoryMetrics.low > 0
                  ? `${inventoryMetrics.low} item${inventoryMetrics.low !== 1 ? "s" : ""} below PAR level`
        ...
        <CardContent>
          <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                {haccpDueChecks.length > 0 ? (
                  <Link href="/haccp/checks">
                    <Badge
                      variant="outline"
                      className="border-amber-500/40 text-amber-400 cursor-pointer"
                    >
                  ...
                  <>
                    <CheckCircle className="h-4 w-4 text-emerald-400" />
                    <span className="text-sm text-emerald-400">All done</span>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <span className="text-sm text-muted-foreground">
                {inventoryMetrics.healthy} of {inventoryMetrics.total} items
                well stocked
              </span>
            </div>
          </div>
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {item.isOut ? (
                          ...
                          <span
                            className={`text-xs ${item.percentFull < 30 ? "text-amber-400 font-medium" : "text-muted-foreground"}`}
                          >
            ...
            {busiestItems.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  Start counting stock to see usage trends
                </p>
              </div>
            ) : (
              <div className="space-y-3">
      ...
      <div className="space-y-3">
        ...
        <div className="grid gap-3 grid-cols-2 sm:grid-cols-3">
          {canCount && (
            <Link href="/stock-count">
              <Card
                className={`group h-full py-4 gap-2 transition-all hover:shadow-md ${isMissingCount ? "border-primary bg-primary/5 hover:bg-primary/10" : "hover:border-primary/30"}`}
              >
                <CardContent className="flex flex-col items-center text-center gap-2">
                  <div
                    className={`rounded-full p-2.5 ${isMissingCount ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground group-hover:bg-primary/10 group-hover:text-primary"} transition-colors`}
                  >
          ...
          {todayAuditItems.length === 0 ? (
            ...
            <div className="space-y-2">
              {todayAuditItems.map((log: AuditLog) => {
                ...
                const categoryColor =
                  CATEGORY_COLORS[log.action_category] ||
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <Badge className={`shrink-0 text-xs ${categoryColor}`}>
