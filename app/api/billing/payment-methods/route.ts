...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only the billing owner can add payment methods
    const isBillingOwnerPost = context.stores.some((s) => s.is_billing_owner);
    if (!isBillingOwnerPost && !context.profile.is_platform_admin) {
      return apiForbidden(
        "Only the billing owner can manage payment methods",
        context.requestId,
      );
    ...
    if (!customerId) {
      return apiBadRequest(
        "No billing account found. Please subscribe to a store first.",
        context.requestId,
      );
    }
