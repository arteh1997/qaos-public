...
import { getWaveAuthUrl } from "@/lib/services/accounting/wave";
import crypto from "crypto";

const STATE_EXPIRY_MINUTES = 10;

/**
 * GET /api/integrations/wave/auth?store_id=xxx
 * Initiates Wave OAuth flow — generates state token and redirects.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const authUrl = getWaveAuthUrl(stateToken);

    return Response.redirect(authUrl, 302);
  } catch (error) {
    return apiError(
      error instanceof Error ? error.message : "Failed to start Wave OAuth",
      { status: 500 },
    );
  }
}
