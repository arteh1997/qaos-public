...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!connection) {
      return apiBadRequest("No Wave connection found", context.requestId);
    }

    const credentials =
      connection.credentials as unknown as AccountingCredentials;
    await revokeWaveToken(credentials);
    ...
    return apiError(
      error instanceof Error ? error.message : "Failed to disconnect Wave",
      { status: 500 },
    );
  }
}
