...
import { exchangeCodeForTokens } from '@/lib/services/accounting/xero'
import { auditLog } from '@/lib/audit'
import { logger } from '@/lib/logger'

/**
 * GET /api/integrations/xero/callback?code=xxx&state=yyy
 * Xero OAuth callback — exchanges code for tokens, stores connection.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    await auditLog(supabase, {
      ...
      details: { tenant_id: credentials.tenant_id },
    })

    return NextResponse.redirect(
      `${baseUrl}/integrations/xero?success=connected`,
      302
    )
  } catch (err) {
    logger.error('Xero OAuth callback error:', { error: err })
    return NextResponse.redirect(
