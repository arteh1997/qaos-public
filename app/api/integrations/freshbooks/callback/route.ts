...
import { exchangeCodeForTokens } from "@/lib/services/accounting/freshbooks";
import { auditLog } from "@/lib/audit";
import { logger } from "@/lib/logger";

/**
 * GET /api/integrations/freshbooks/callback?code=xxx&state=yyy
 * FreshBooks OAuth callback — exchanges code for tokens, stores connection.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    await auditLog(supabase, {
      ...
      details: { account_id: credentials.tenant_id },
    });

    return NextResponse.redirect(
      `${baseUrl}/integrations/freshbooks?success=connected`,
      302,
    );
  } catch (err) {
    logger.error("FreshBooks OAuth callback error:", { error: err });
    return NextResponse.redirect(
