...
import { getSageAuthUrl } from "@/lib/services/accounting/sage";
import crypto from "crypto";

const STATE_EXPIRY_MINUTES = 10;

/**
 * GET /api/integrations/sage/auth?store_id=xxx
 * Initiates Sage OAuth flow — generates state token and redirects.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const authUrl = getSageAuthUrl(stateToken);

    return Response.redirect(authUrl, 302);
  } catch (error) {
    return apiError(
      error instanceof Error ? error.message : "Failed to start Sage OAuth",
      { status: 500 },
    );
  }
}
