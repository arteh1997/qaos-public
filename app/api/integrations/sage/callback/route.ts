...
import { exchangeCodeForTokens } from "@/lib/services/accounting/sage";
import { auditLog } from "@/lib/audit";
import { logger } from "@/lib/logger";

/**
 * GET /api/integrations/sage/callback?code=xxx&state=yyy
 * Sage OAuth callback — exchanges code for tokens, stores connection.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    await auditLog(supabase, {
      ...
      storeId,
      action: "sage.connected",
      details: {},
    });

    return NextResponse.redirect(
      `${baseUrl}/integrations/sage?success=connected`,
      302,
    );
  } catch (err) {
    logger.error("Sage OAuth callback error:", { error: err });
    return NextResponse.redirect(
