...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!connection) {
      return apiBadRequest("No Sage connection found", context.requestId);
    }

    const credentials =
      connection.credentials as unknown as AccountingCredentials;
    await revokeSageToken(credentials);
    ...
    return apiError(
      error instanceof Error ? error.message : "Failed to disconnect Sage",
      { status: 500 },
    );
  }
}
