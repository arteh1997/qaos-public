...
import { getZohoBooksAuthUrl } from "@/lib/services/accounting/zoho-books";
import crypto from "crypto";

const STATE_EXPIRY_MINUTES = 10;

/**
 * GET /api/integrations/zoho-books/auth?store_id=xxx
 * Initiates Zoho Books OAuth flow — generates state token and redirects.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const authUrl = getZohoBooksAuthUrl(stateToken);
    ...
    return apiError(
      error instanceof Error
        ? error.message
        : "Failed to start Zoho Books OAuth",
      { status: 500 },
    );
  }
}
