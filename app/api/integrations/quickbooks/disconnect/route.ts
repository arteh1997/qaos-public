...
import { revokeQuickBooksToken } from '@/lib/services/accounting/quickbooks'
import { auditLog } from '@/lib/audit'
import type { AccountingCredentials } from '@/lib/services/accounting/types'

/**
 * POST /api/integrations/quickbooks/disconnect
 * Disconnect QuickBooks — revoke token and deactivate connection.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!connection) {
      return apiBadRequest('No QuickBooks connection found', context.requestId)
    }

    // Revoke token (best effort)
    const credentials = connection.credentials as unknown as AccountingCredentials
    await revokeQuickBooksToken(credentials)
