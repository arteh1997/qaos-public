...
import { getQuickBooksAuthUrl } from '@/lib/services/accounting/quickbooks'
import { QUICKBOOKS_OAUTH_CONFIG } from '@/lib/validations/accounting'
import crypto from 'crypto'

/**
 * GET /api/integrations/quickbooks/auth?store_id=xxx
 * Initiates QuickBooks OAuth flow — generates state token and redirects.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const expiresAt = new Date(
      Date.now() + QUICKBOOKS_OAUTH_CONFIG.stateExpiryMinutes * 60 * 1000
    ).toISOString()
    ...
    const authUrl = getQuickBooksAuthUrl(stateToken)

    return Response.redirect(authUrl, 302)
  } catch (error) {
    return apiError(
      error instanceof Error ? error.message : 'Failed to start QuickBooks OAuth',
      { status: 500 }
    )
  }
}
