...
import { getMyobAuthUrl } from "@/lib/services/accounting/myob";
import crypto from "crypto";

const STATE_EXPIRY_MINUTES = 10;

/**
 * GET /api/integrations/myob/auth?store_id=xxx
 * Initiates MYOB OAuth flow — generates state token and redirects.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const authUrl = getMyobAuthUrl(stateToken);

    return Response.redirect(authUrl, 302);
  } catch (error) {
    return apiError(
      error instanceof Error ? error.message : "Failed to start MYOB OAuth",
      { status: 500 },
    );
  }
}
