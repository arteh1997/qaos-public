...
import { exchangeCodeForTokens } from "@/lib/services/accounting/myob";
import { auditLog } from "@/lib/audit";
import { logger } from "@/lib/logger";

/**
 * GET /api/integrations/myob/callback?code=xxx&state=yyy
 * MYOB OAuth callback — exchanges code for tokens, stores connection.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    await auditLog(supabase, {
      ...
      details: { company_file_uri: credentials.tenant_id },
    });

    return NextResponse.redirect(
      `${baseUrl}/integrations/myob?success=connected`,
      302,
    );
  } catch (err) {
    logger.error("MYOB OAuth callback error:", { error: err });
    return NextResponse.redirect(
