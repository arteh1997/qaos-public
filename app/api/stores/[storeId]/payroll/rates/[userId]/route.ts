...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!canManageStore(context, storeId)) {
      return apiForbidden('You do not have permission to update payroll rates', context.requestId)
    }
    ...
    const { data: storeUser } = await context.supabase
      .from('store_users')
      .select('id, user_id, hourly_rate, role, user:profiles!store_users_user_id_fkey(full_name, email)')
      .eq('store_id', storeId)
      .eq('user_id', userId)
      .single()
    ...
    if (storeUser.role === 'Owner') {
      return apiForbidden('Cannot set hourly rate for an Owner', context.requestId)
    }
    ...
    if (callerRecord?.role === 'Manager' && storeUser.role !== 'Staff') {
      return apiForbidden('Managers can only set rates for Staff members', context.requestId)
    }
    ...
    await auditLog(admin, {
      ...
      details: {
        employeeId: userId,
        employeeName: (storeUser.user as any)?.full_name,
        previousRate,
        newRate: hourly_rate,
      },
      request,
    ...
    return apiError(error instanceof Error ? error.message : 'Failed to update hourly rate')
  }
}
