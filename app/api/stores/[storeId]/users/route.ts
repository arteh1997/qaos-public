...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    return apiError(error instanceof Error ? error.message : 'Failed to fetch store users')
  }
}
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (existing) {
      return apiBadRequest('User is already a member of this store', context.requestId)
    }
    ...
    return apiError(error instanceof Error ? error.message : 'Failed to add user')
  }
}
