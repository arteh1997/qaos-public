...
const csvRowSchema = z.object({
  ...
  par_level: z.coerce
    .number()
    .positive("Par level must be positive")
    .optional(),
...
export async function POST(
  ...
  try {
    ...
    // Verify user has permission to manage inventory at this store
    ...
    if (!storeUser || !["Owner", "Manager"].includes(storeUser.role)) {
      return apiForbidden(
        "Access denied. Only Owners and Managers can import inventory.",
        context.requestId,
      );
    }
    ...
    if (!file) {
      return apiBadRequest("No file uploaded", context.requestId);
    }
    ...
    if (missingHeaders.length > 0) {
      return apiBadRequest(
        `Missing required columns: ${missingHeaders.join(", ")}. Expected: ${expectedHeaders.join(", ")}`,
        context.requestId,
      );
    }
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const rowData = {
        ...
        current_stock:
          currentStockIndex >= 0 && row[currentStockIndex]
            ? row[currentStockIndex]
            : undefined,
        par_level:
          parLevelIndex >= 0 && row[parLevelIndex]
            ? row[parLevelIndex]
            : undefined,
        cost_per_unit:
          costIndex >= 0 && row[costIndex] ? row[costIndex] : undefined,
      };
    ...
    if (errorRows.length > 0) {
      return apiBadRequest(
        `Found ${errorRows.length} invalid row(s)`,
        context.requestId,
      );
    }

    if (validItems.length === 0) {
      return apiBadRequest("No valid items to import", context.requestId);
    }
    ...
    if (insertError) {
      ...
      return apiError("Failed to import items");
    }
    ...
    if (insertedItems && insertedItems.length > 0) {
      ...
      if (inventoryError) {
        logger.error("[Import] Store inventory error:", {
          error: inventoryError,
        });
        return apiError("Failed to link items to store");
      }
    }
    ...
    // Audit log the bulk import (admin client required for audit writes)
