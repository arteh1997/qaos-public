...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (parsed.data.entity_id) {
      ...
      if (invoices) {
        ...
        const syncedIds = new Set(
          (existingSyncs || []).map((s) => s.entity_id),
        );
        const unsyncedInvoices = invoices.filter(
          (inv) => !syncedIds.has(inv.id),
        );
    ...
    await supabase
      ...
      .update({
        ...
        sync_error: hasErrors
          ? `${results.filter((r) => !r.success).length} sync(s) failed`
          : null,
    ...
    return apiError(error instanceof Error ? error.message : "Failed to sync", {
      status: 500,
    });
  }
}
...
async function syncSingleInvoice(
  ...
  try {
    ...
    // Get line items with category from linked inventory item
    ...
    const bill: AccountingBill = {
      ...
      line_items: (lineItems || [])
        ...
        .map((li) => {
          const category = (li.inventory_items as { category?: string } | null)
            ?.category;
          const accountCode =
            (category && glMappings[category]) ||
            glMappings["_default"] ||
            defaultAccountCode;
