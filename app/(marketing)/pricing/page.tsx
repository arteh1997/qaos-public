          ...
          <div className="container mx-auto px-6 lg:px-8">
                ...
                <ScrollReveal key={index} delay={index * 80}>
                  <div className="rounded-2xl border border-border/60 bg-card p-6 lg:p-8 shadow-card">
                    <div className="flex items-start gap-4">
                      <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#2d7a4f]/10 text-[#2d7a4f] flex-shrink-0">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm font-medium text-[#2d7a4f]">
                          Typical savings: {formatSaving(item.savingMultiplier[0])} - {formatSaving(item.savingMultiplier[1])}/month
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <ScrollReveal delay={350}>
              <div className="mt-12 text-center">
                <div className="inline-flex items-center gap-2 rounded-full border border-[#2d7a4f]/30 bg-[#2d7a4f]/10 px-5 py-2.5 text-sm font-medium text-[#2d7a4f]">
                  <PiggyBank className="h-4 w-4" />
                  Average total savings: {formatSaving(3.5)} - {formatSaving(7.7)}/month per location
                </div>
              </div>
            </ScrollReveal>
          </div>
              ...
              <div className="mx-auto max-w-xl">
                ...
                <div className="relative rounded-3xl border-2 border-primary/30 p-10 lg:p-12 shadow-xl glass gradient-border">
                  ...
                  <div className="text-center mb-10">
                    <span className="text-5xl sm:text-6xl font-bold text-foreground tracking-tight">{tier.symbol}{displayPrice}</span>
                  ...
                  <ul className="mb-10 space-y-3">
                    {features.map((feature, index) => (
                      <li key={index} className="flex items-center gap-3">
                        <div className="flex h-5 w-5 items-center justify-center rounded-full bg-[#2d7a4f]/10">
                ...
                <ScrollReveal delay={200}>
                  <div className="mt-10 rounded-2xl border border-border/60 bg-card p-8 shadow-card">
                    ...
                    <p className="text-sm text-muted-foreground text-center mb-6">
                      The more stores you add, the less you pay. Discount applies to your entire bill.
                    </p>

                    {/* Discount table */}
                    <div className="space-y-3 mb-6">
                      <div className="flex items-center justify-between rounded-xl bg-muted/30 px-4 py-3">
                        <span className="text-sm text-muted-foreground">1-4 stores</span>
                        <span className="text-sm font-medium text-foreground">{tier.symbol}{displayPrice}/store/month — full price</span>
                      </div>
                      {VOLUME_DISCOUNTS.map((vd, i) => {
                        ...
                        const effectivePerStore = (bill.total / vd.minStores / 100).toLocaleString(tier.locale, { minimumFractionDigits: 0, maximumFractionDigits: 0 })
                        return (
                          <div key={i} className="flex items-center justify-between rounded-xl bg-[#2d7a4f]/5 border border-[#2d7a4f]/20 px-4 py-3">
                            <div>
                            ...
                            <div className="text-right">
                              <div className="text-sm font-semibold text-foreground">{tier.symbol}{totalDisplay}/mo total</div>
                              <div className="text-xs text-[#2d7a4f]">{tier.symbol}{effectivePerStore}/store · save {tier.symbol}{savedDisplay}</div>
                            </div>
                          </div>
                        )
                      })}
                    ...
                    <p className="text-xs text-muted-foreground text-center">
                      Example: 10 stores at {tier.symbol}{displayPrice} = {tier.symbol}{(tier.amount / 100 * 10).toLocaleString(tier.locale, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}.
                      With 20% discount you pay {tier.symbol}{(calculateMonthlyBill(10, tier.currency.toUpperCase()).total / 100).toLocaleString(tier.locale, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}/month.
                    </p>
                  </div>
                </ScrollReveal>
                ...
                <ScrollReveal delay={300}>
                  <div className="mt-8 rounded-2xl border border-[#2d7a4f]/30 bg-[#2d7a4f]/5 p-8 text-center">
                    ...
                    <p className="text-sm text-muted-foreground mt-3">
                      That&apos;s a {formatSaving(3.5)} - {formatSaving(7.7)} return on your investment
                    </p>
                  </div>
                </ScrollReveal>
              </div>
