...
function ShiftsPageContent() {
  ...
  const [editingClockTimesShift, setEditingClockTimesShift] =
  ...
  } = useShifts(currentStoreId || null, null, dateRange);
  ...
  const viewIncludesToday = useMemo(() => {
    ...
    const to = pickerRange.to
      ? endOfDay(pickerRange.to)
      : endOfDay(pickerRange.from);
  ...
  const handlePrevWeek = () => {
    ...
    setPickerRange({
      from: prevWeekStart,
      to: endOfWeek(prevWeekStart, { weekStartsOn: 1 }),
    });
  };
      ...
      {!isLoading && shifts.length > 0 && (
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <StatsCard
            ...
            variant={
              stats.attendanceRate !== null && stats.attendanceRate < 80
                ? "warning"
                : "default"
            }
          />
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center gap-2.5">
              ...
              <CardTitle className="text-sm font-semibold">
                {activeShifts.length} team member
                {activeShifts.length !== 1 ? "s" : ""} on shift right now
              </CardTitle>
            </div>
          </CardHeader>
            ...
            {(() => {
              ...
              const rangeLabel =
                pickerRange?.from && pickerRange?.to
                  ? format(pickerRange.from, "yyyy-MM-dd") ===
                    format(pickerRange.to, "yyyy-MM-dd")
        ...
        <div className="space-y-4">
          {groupedShifts.map(([dateKey, dayShifts]) => {
            ...
            return (
              <Card
                key={dateKey}
                className={cn("bg-card", isNow && "border-emerald-500/40")}
              >
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      {formatHours(Math.round(dayHours * 10) / 10)}
                    </p>
                  </div>
                </CardHeader>
                <CardContent>
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuItem
                                  onClick={() =>
                                    setEditingClockTimesShift(shift)
                                  }
                                >
