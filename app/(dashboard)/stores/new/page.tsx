  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || errorData.error || "Failed to create store",
        );
      }
      ...
      const subscriptionResponse = await csrfFetch(
      ...
      if (!subscriptionResponse.ok) {
        ...
        toast.error(
          "Store created but billing setup incomplete. Please check billing settings.",
        );
      }
      ...
      {isFirstStore && (
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span
              className={`text-sm font-medium ${step === "billing" || paymentMethodId ? "text-foreground" : "text-muted-foreground"}`}
            >
              Billing
            </span>
          </button>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Add a card to start your 30-day free trial. You won&apos;t be
          ...
          <div className="rounded-xl border bg-card p-5 space-y-3">
            ...
            <div className="grid gap-2.5">
              {[
                {
                  icon: Clock,
                  text: "Set up operating hours and shift patterns",
                },
                {
                  icon: Package,
                  text: "Add inventory items or import from another location",
                },
                ...
                <div className="mt-4 space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">
                      {currentStoreCount} current location
                      {currentStoreCount !== 1 ? "s" : ""}
                    </span>
                    <span className="tabular-nums">
                      £{currentStoreCount * 299}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
