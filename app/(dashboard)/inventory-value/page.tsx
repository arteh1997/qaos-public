...
export default function StockValuePage() {
  ...
  const handleSaveCost = async (inventoryItemId: string) => {
    ...
    setSaving(true)
    try {
      const res = await csrfFetch(`/api/stores/${storeId}/inventory/${inventoryItemId}`, {
        method: 'PATCH',
  ...
  const scrollToTableAndEdit = useCallback((itemId: string) => {
  ...
  const data = useMemo(() => {
    if (!inventory) return {
      items: [] as { id: string; name: string; category: string; unit: string; quantity: number; unit_cost: number; total_value: number }[],
      total: 0,
      byCategory: [] as { category: string; count: number; value: number; percentage: number }[],
      missingCostCount: 0,
      itemsWithCost: 0,
      missingCostItems: [] as { id: string; name: string; category: string; unit: string; quantity: number; unit_cost: number; total_value: number }[],
      topItems: [] as { id: string; name: string; category: string; unit: string; quantity: number; unit_cost: number; total_value: number }[],
    ...
    const byCategory = Array.from(categoryMap.entries())
      .map(([category, d]) => ({ category, ...d, percentage: total > 0 ? (d.value / total) * 100 : 0 }))
      .sort((a, b) => b.value - a.value)

    const categories = Array.from(categoryMap.keys()).sort()

    return { items, total, byCategory, missingCostCount, itemsWithCost, missingCostItems, topItems, categories }
  }, [inventory])
  ...
  const tableItems = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'quantity': return m * (a.quantity - b.quantity)
        case 'unit_cost': return m * (a.unit_cost - b.unit_cost)
        case 'total_value': return m * (a.total_value - b.total_value)
        default: return 0
      }
    })
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground">Only Owners and Managers can view stock value.</p>
        </CardContent>
      </Card>
    )
  }
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground">Select a store to view stock value.</p>
        </CardContent>
      </Card>
    )
  }
  ...
  const remainingValue = remainingCategories.reduce((sum, c) => sum + c.value, 0)

  return (
    <div className="space-y-6">
      {/* ── Header ── */}
      <PageHeader title="Inventory Value" description="Total stock value across all items">
      ...
      <div className="grid gap-4 grid-cols-2 sm:grid-cols-3">
        ...
        <StatsCard
          ...
          description={topCategory ? `${formatGBP(topCategory.value)} — ${Math.round(topCategory.percentage)}% of total` : 'No data yet'}
          icon={<TrendingUp className="h-4 w-4" />}
        />
        <StatsCard
          title="Needs Pricing"
          value={data.missingCostCount}
          description={data.missingCostCount === 0 ? 'All costs set' : `${data.missingCostCount} item${data.missingCostCount !== 1 ? 's' : ''} need a cost`}
          icon={data.missingCostCount === 0 ? <CheckCircle2 className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4" />}
          ...
          <CardContent className="space-y-3">
            {shownCategories.map(cat => (
              <div key={cat.category} className="flex items-center gap-3">
                <span className="text-sm font-medium w-32 sm:w-40 truncate shrink-0">{cat.category}</span>
                ...
                <span className="text-sm font-semibold font-mono w-16 text-right shrink-0">{formatGBP(cat.value)}</span>
              </div>
            ))}
            {remainingCategories.length > 0 && (
              <p className="text-xs text-muted-foreground pt-1">
                and {remainingCategories.length} more categor{remainingCategories.length === 1 ? 'y' : 'ies'} ({formatGBP(remainingValue)} total)
              </p>
            )}
          </CardContent>
        </Card>
      ...
      {data.topItems.length > 0 && (
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base font-semibold">Most Valuable Stock</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-0 divide-y">
              {data.topItems.map((item, index) => (
                <div key={item.id} className="flex items-center gap-3 py-2.5 first:pt-0 last:pb-0">
                  ...
                  <span className="text-sm font-semibold font-mono shrink-0">{formatGBP(item.total_value)}</span>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <Card className="border-amber-300">
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground mb-3">
              These items have no cost set. Tap to set a price so your stock value is accurate.
            </p>
            <div className="space-y-0 divide-y">
              {data.missingCostItems.map(item => (
                ...
                >
                  <div className="min-w-0">
                    <p className="text-sm font-medium truncate">{item.name}</p>
                    <p className="text-xs text-muted-foreground">{item.quantity} {item.unit} in stock</p>
                  </div>
            ...
            {data.missingCostCount > 5 && (
              <p className="text-xs text-muted-foreground mt-3">
                +{data.missingCostCount - 5} more item{data.missingCostCount - 5 !== 1 ? 's' : ''} without costs
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className="text-sm text-muted-foreground">{tableItems.length} item{tableItems.length !== 1 ? 's' : ''}</span>
            </div>
              ...
              <>
                ...
                <div className="space-y-2 md:hidden">
                  {tableItems.map(item => (
                    <div
                      key={item.id}
                      className={`flex items-center justify-between p-3 rounded-lg ${item.unit_cost === 0 ? 'bg-amber-50/50' : 'bg-muted/50'}`}
                    >
                      ...
                      <div className="text-right ml-3">
                        {editingId === item.id ? (
                          ...
                          <button
                            onClick={() => handleStartEdit(item.id, item.unit_cost)}
                            className="text-right"
                          >
                            {item.unit_cost > 0 ? (
                              <>
                                <p className="font-semibold text-sm font-mono">{formatGBP(item.total_value, true)}</p>
                                <p className="text-xs text-muted-foreground">{formatGBP(item.unit_cost, true)} each</p>
                              </>
                            ) : (
                ...
                <div className="hidden md:block">
                  <Table>
                    ...
                    <TableBody>
                      {tableItems.map(item => (
                        <TableRow key={item.id} className={item.unit_cost === 0 ? 'bg-amber-50/50' : undefined}>
                          ...
                          <TableCell className="text-right">
                            {editingId === item.id ? (
                              ...
                              >
                                {item.unit_cost > 0 ? (
                                  <span className="font-mono">{formatGBP(item.unit_cost, true)}</span>
                                ) : (
                          ...
                          <TableCell className="text-right font-mono font-semibold">
                            {item.unit_cost > 0 ? formatGBP(item.total_value, true) : '—'}
                          </TableCell>
                        </TableRow>
                      ))}
                      {filteredTotal > 0 && (
                        <TableRow className="bg-muted/50 font-bold border-t-2">
                          <TableCell colSpan={4} className="text-right">Total Stock Value</TableCell>
                          <TableCell className="text-right font-mono">{formatGBP(filteredTotal, true)}</TableCell>
                        </TableRow>
                      )}
                    </TableBody>
                  </Table>
