...
export default function HACCPCorrectiveActionsPage() {
  ...
  const { data: actions, isLoading } = useHACCPCorrectiveActions(storeId, queryParams)
  ...
  if (!storeId) {
    return (
      <div className="space-y-6">
        <PageHeader title="Corrective Actions" description="Track and resolve food safety issues">
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          {isLoading ? (
            ...
            <EmptyState
              ...
              title={unresolvedOnly ? 'No open actions' : 'No corrective actions'}
              description={
                unresolvedOnly
                  ? 'All corrective actions have been resolved.'
                  : 'Create a corrective action when a food safety issue is identified.'
              }
              action={isManagement ? { label: 'Create Action', onClick: () => setShowCreateDialog(true), icon: Plus } : undefined}
            />
          ) : (
            <>
              {/* Mobile card view */}
              <div className="sm:hidden space-y-3">
                {actions.map((action) => (
                  <div
                    key={action.id}
                    className={`border rounded-lg p-3 space-y-2 ${action.status === 'open' ? 'border-amber-200' : ''}`}
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {action.status === 'open' ? (
                        <Badge variant="secondary" className="bg-amber-50 text-amber-700">Open</Badge>
                      ) : (
                        <Badge variant="secondary" className="bg-emerald-50 text-emerald-700">Resolved</Badge>
                      )}
                    </div>
                    ...
                    {action.created_by_name && (
                      <p className="text-xs text-muted-foreground">Created by {action.created_by_name}</p>
                    )}
                    {action.resolved_by_name && (
                      <p className="text-xs text-muted-foreground">
                        Resolved by {action.resolved_by_name} on{' '}
                        {action.resolved_at
                          ? new Date(action.resolved_at).toLocaleDateString('en-GB', {
                    ...
                    {action.status === 'open' && isManagement && (
                      <Button
                        ...
                        onClick={() => { setResolvingActionId(action.id); setResolveActionTaken('') }}
                      >
              ...
              <div className="hidden sm:block">
                <Table>
                  ...
                  <TableBody>
                    {actions.map((action) => (
                      <TableRow key={action.id} className={action.status === 'open' ? 'bg-amber-50/30' : ''}>
                        ...
                        <TableCell className="text-muted-foreground">{action.created_by_name ?? '-'}</TableCell>
      ...
      <Dialog open={showCreateDialog} onOpenChange={(open) => { setShowCreateDialog(open); if (!open) setCreateDescription('') }}>
        <DialogContent>
          <DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setShowCreateDialog(false); setCreateDescription('') }}>
              Cancel
            </Button>
            <Button
      ...
      <Dialog open={!!resolvingActionId} onOpenChange={(open) => { if (!open) { setResolvingActionId(null); setResolveActionTaken('') } }}>
        <DialogContent>
          <DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setResolvingActionId(null); setResolveActionTaken('') }}>
              Cancel
            </Button>
            <Button
