  ...
  if (!storeId) {
    return (
      <div className="space-y-6">
        <PageHeader title="HACCP Compliance" description="Food safety management and monitoring">
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <Card className={getComplianceBg(dashboard.compliance_score)}>
            <CardContent className="pt-4">
              ...
              <p className={`text-2xl font-bold mt-1 ${getComplianceColor(dashboard.compliance_score)}`}>
          ...
          <Card>
            <CardContent className="pt-4">
              ...
              <p className="text-2xl font-bold mt-1">{dashboard.today.total_checks}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4">
              ...
              <p className={`text-2xl font-bold mt-1 ${dashboard.today.out_of_range_temps > 0 ? 'text-destructive' : ''}`}>
          ...
          <Card>
            <CardContent className="pt-4">
              ...
              <p className={`text-2xl font-bold mt-1 ${dashboard.unresolved_corrective_actions > 0 ? 'text-amber-600' : ''}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {dashboard.due_checks.map((check) => (
                <Link key={check.id} href="/haccp/checks">
                  <div className="flex items-center justify-between border border-amber-200 bg-white rounded-lg p-3 hover:bg-amber-50 transition-colors cursor-pointer">
                    <div>
                      <p className="font-medium text-sm">{check.name}</p>
                      <p className="text-xs text-muted-foreground">{getFrequencyLabel(check.frequency)}</p>
                    </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm font-medium">Recent Temperature Alerts</CardTitle>
            <Link href="/haccp/temperatures">
              <Button variant="ghost" size="sm" className="text-xs">
                View All
              </Button>
            ...
            <>
              ...
              <div className="sm:hidden space-y-3">
                {dashboard.recent_temp_alerts.map((log) => (
                  <div key={log.id} className="border border-destructive/20 rounded-lg p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-muted-foreground">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-sm font-bold text-destructive">{log.temperature_celsius}°C</span>
                      </div>
                    </div>
        ...
        <Link href="/haccp/templates">
          <Card className="hover:bg-muted/50 transition-colors cursor-pointer">
            <CardContent className="pt-4 flex flex-col items-center text-center gap-2">
              ...
              <p className="text-xs text-muted-foreground">Manage check templates</p>
            </CardContent>
          </Card>
        </Link>
        <Link href="/haccp/checks">
          <Card className="hover:bg-muted/50 transition-colors cursor-pointer">
            <CardContent className="pt-4 flex flex-col items-center text-center gap-2">
              ...
              <p className="text-xs text-muted-foreground">Submit and view checks</p>
            </CardContent>
          </Card>
        </Link>
        <Link href="/haccp/temperatures">
          <Card className="hover:bg-muted/50 transition-colors cursor-pointer">
            <CardContent className="pt-4 flex flex-col items-center text-center gap-2">
              ...
              <p className="text-xs text-muted-foreground">Log and monitor temps</p>
            </CardContent>
          </Card>
        </Link>
