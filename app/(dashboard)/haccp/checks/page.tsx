...
export default function HACCPChecksPage() {
  ...
  const { data: templates, isLoading: loadingTemplates } =
    useHACCPTemplates(storeId);
  const { data: checks, isLoading: loadingChecks } = useHACCPChecks(
    storeId,
    checksParams,
  );
  ...
  const handleSelectTemplate = (templateId: string) => {
    ...
    setResponses(
      template.items.map((item) => ({
        ...
        item_type: item.type,
        value:
          item.type === "yes_no" ? true : item.type === "temperature" ? 0 : "",
                  ...
                  <div key={index} className="border rounded-lg p-3">
                      ...
                      <div className="flex items-center gap-2">
                        <Switch
                          ...
                          onCheckedChange={(checked) =>
                            handleUpdateResponse(index, checked)
                          }
                        />
                    ...
                    {response.item_type === "text" && (
                      <Input
                        ...
                        onChange={(e) =>
                          handleUpdateResponse(index, e.target.value)
                        }
                      />
                    )}
                  </div>
