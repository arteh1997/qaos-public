...
export default function AccountingIntegrationsPage() {
  ...
  const { connections, isLoading } = useAccountingConnections(
    storeId || undefined,
  );
  ...
  const handleConnect = (provider: string) => {
    ...
    window.location.assign(
      `/api/integrations/${provider}/auth?store_id=${storeId}`,
    );
  };
        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filteredProviders.map(
              ({ provider, name, description, icon, region }) => {
                ...
                return (
                  <IntegrationCard
                    ...
                    name={name}
                    description={
                      connection ? description : `${description} — ${region}`
                    }
                    icon={icon}
          ...
          <Card>
            ...
            <CardContent>
              <ol className="text-sm text-muted-foreground space-y-2 list-decimal list-inside">
                <li>
                  Connect your accounting software using the buttons above
                </li>
                <li>Purchase orders are automatically synced as bills</li>
                <li>
                  Supplier details are matched to contacts in your accounting
                  system
                </li>
