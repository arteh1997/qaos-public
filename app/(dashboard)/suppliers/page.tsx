...
import {
  Truck, Plus, Search, Pencil, Trash2, Package, ArrowLeft,
  PoundSterling, FileText, ShoppingCart, Clock, KeyRound,
} from 'lucide-react'
...
export default function SuppliersPage() {
  ...
  const {
    ...
    fetchOrder: fetchOrderDetail, updateOrder, deleteOrder, receiveItems, isSubmitting: submittingReceive,
  ...
  const activeOrders = useMemo(() =>
    orders.filter(o => !['cancelled', 'received'].includes(o.status)), [orders])
  const pendingDeliveries = useMemo(() =>
    orders.filter(o => ['awaiting_delivery', 'partial'].includes(o.status)), [orders])
  const openOrderValue = useMemo(() =>
    activeOrders.reduce((sum, o) => sum + o.total_amount, 0), [activeOrders])
  ...
  if (role !== 'Owner' && role !== 'Manager') {
    return (
      <div className="space-y-6">
        <PageHeader title="Suppliers & Orders" description="This feature is only available to Owners and Managers.">
  ...
  if (!storeId) {
    return (
      <div className="space-y-6">
        <PageHeader title="Suppliers & Orders" description="Select a store to manage suppliers.">
        <PageGuide pageKey="suppliers" />
      </PageHeader>
      </div>
    )
  ...
  const handleStatusChange = async (status: string) => {
    try {
      await updateOrder({ status: status as Parameters<typeof updateOrder>[0]['status'] })
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 grid-cols-2 sm:grid-cols-4">
        ...
        <StatsCard
          ...
          description={activeOrders.length > 0 ? `${orders.filter(o => o.status === 'open').length} open` : 'No active orders'}
        ...
        <StatsCard
          ...
          description={pendingDeliveries.length > 0 ? 'In transit or partial' : 'All delivered'}
                ...
                return (
                  <div key={supplier.id} className="border rounded-lg p-3 space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {activeCount > 0 && (
                        <Badge variant="secondary" className="bg-blue-50 text-blue-700 shrink-0">
                          {activeCount} order{activeCount !== 1 ? 's' : ''}
                        </Badge>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" className="flex-1" onClick={() => handleQuickOrder(supplier.id)}>
                        <ShoppingCart className="h-3.5 w-3.5 mr-1" />
                        Order
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setPortalSupplier(supplier)} title="Portal Access">
                        <KeyRound className="h-3.5 w-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setEditingSupplier(supplier)}>
                        <Pencil className="h-3.5 w-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setDeletingSupplier(supplier)}>
                        ...
                        <TableRow key={supplier.id} className="hover:bg-muted/30 transition-colors">
                          ...
                          <TableCell>
                            <div className="flex items-center gap-1">
                              <Button variant="outline" size="sm" onClick={() => handleQuickOrder(supplier.id)}>
                                <ShoppingCart className="h-3.5 w-3.5 mr-1" />
                                Order
                              </Button>
                              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setPortalSupplier(supplier)} title="Portal Access">
          ...
          {selectedPOId && selectedOrder ? (
            ...
            <>
              <div className="flex items-center justify-between gap-3">
                ...
                <Button onClick={() => { setPreselectedSupplierId(undefined); setShowPOForm(true) }}>
                  <Plus className="h-4 w-4 sm:mr-2" />
              ...
              {loadingOrders ? (
                ...
                <EmptyState
                  ...
                  action={statusFilter === 'all'
                    ? { label: 'Create Order', onClick: () => { setPreselectedSupplierId(undefined); setShowPOForm(true) }, icon: Plus }
                    : undefined
                  }
                />
              ) : (
                <>
                  ...
                  <div className="sm:hidden space-y-2">
                    {filteredOrders.map(order => {
                      const deliveryText = formatDeliveryDate(order.expected_delivery_date)
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <span className="font-medium text-sm">{order.po_number}</span>
                          ...
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <span className={isOverdue ? 'text-destructive text-xs font-medium' : 'text-xs text-muted-foreground'}>
                              {deliveryText}
                            </span>
                          </div>
                        </div>
      ...
      {portalSupplier && (
        <PortalTokenManager
          open={!!portalSupplier}
          onOpenChange={open => { if (!open) setPortalSupplier(null) }}
...
function PODetailView({
  ...
  const totalOrdered = order.items?.reduce((sum, i) => sum + i.quantity_ordered, 0) ?? 0
  const totalReceived = order.items?.reduce((sum, i) => sum + (i.quantity_received ?? 0), 0) ?? 0
  const receiveProgress = totalOrdered > 0 ? Math.round((totalReceived / totalOrdered) * 100) : 0
  const canReceive = ['open', 'awaiting_delivery', 'partial'].includes(order.status)
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div>
            ...
            <p className="text-sm font-medium mt-0.5">{formatDate(order.order_date)}</p>
          </div>
          <div>
            ...
            <p className="text-sm font-medium mt-0.5">
              ...
              {order.expected_delivery_date && !isTerminal && (
                <span className={`ml-1 text-xs ${formatDeliveryDate(order.expected_delivery_date).includes('overdue') ? 'text-destructive' : 'text-muted-foreground'}`}>
          ...
          <div>
            ...
            <p className="text-sm font-medium mt-0.5">{order.items?.length ?? 0} line items</p>
          </div>
          <div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-medium">
                {totalReceived} / {totalOrdered} units received ({receiveProgress}%)
              </span>
            </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right hidden sm:table-cell">Per Unit</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {order.items?.map(item => {
              const lineTotal = item.quantity_ordered * item.unit_price
              const fullyReceived = (item.quantity_received ?? 0) >= item.quantity_ordered
              return (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-right">
                    <span className={fullyReceived ? 'text-emerald-600 font-medium' : ''}>
                  ...
                  <TableCell className="text-right text-muted-foreground text-sm hidden sm:table-cell">
                    £{item.unit_price.toFixed(2)}/{item.inventory_item?.unit_of_measure ?? 'unit'}
                  </TableCell>
                </TableRow>
              )
            })}
