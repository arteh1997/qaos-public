...
const ForecastChart = dynamic(
  () =>
    import("@/components/charts/ForecastChart").then((mod) => ({
...
const StockProjectionChart = dynamic(
  () =>
    import("@/components/charts/ForecastChart").then((mod) => ({
      default: mod.StockProjectionChart,
    })),
  { loading: () => <Skeleton className="h-64 w-full" /> },
);
const WeekdayPatternChart = dynamic(
  () =>
    import("@/components/charts/ForecastChart").then((mod) => ({
      default: mod.WeekdayPatternChart,
    })),
  { loading: () => <Skeleton className="h-64 w-full" /> },
);
import {
...
function TrendIcon({ direction }: { direction: string }) {
  ...
  if (direction === "decreasing")
    return <TrendingDown className="size-4 text-emerald-400" />;
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-foreground leading-relaxed">
              Analysing{" "}
              <span className="font-semibold">{data.summary.total} items</span>{" "}
              across <span className="font-semibold">{days} days</span> of
              history.
              {urgent > 0 ? (
                <>
                  {" "}
              ...
              <div className="flex items-center justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      variant="secondary"
                      className={RISK_BADGE[item.riskLevel]}
                    >
                      {item.riskLevel}
                    </Badge>
                  </div>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    {item.unitCost > 0 && (
                      <p className="text-xs text-muted-foreground">
                        Est.{" "}
                        {formatCurrency(
                          item.suggestedOrderQuantity * item.unitCost,
                        )}
                      </p>
                    )}
                    {item.suggestedOrderDate && (
                      <p className="text-xs text-destructive font-medium mt-0.5">
                        {new Date(
                          item.suggestedOrderDate + "T12:00:00",
...
function ItemDetailView({
  ...
  const maxRatio =
    avgPattern > 0
      ? (item.weekdayPattern[maxDayIdx] / avgPattern).toFixed(1)
      : "0";
  const minRatio =
    avgPattern > 0
      ? (item.weekdayPattern[minDayIdx] / avgPattern).toFixed(1)
      : "0";
      ...
      {data && !selectedItem && (
        <>
          ...
          {data.forecasts.length === 0 && (
            <Card>
              <CardContent className="pt-6 text-center text-muted-foreground">
                No items found. Add inventory items and record stock activity to
