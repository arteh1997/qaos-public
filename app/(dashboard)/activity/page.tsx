...
const CATEGORY_GROUPS: {
  ...
  { value: "payroll", label: "Payroll", categories: ["payroll"] },
  {
    value: "system",
    label: "System",
    categories: ["auth", "store", "settings", "report"],
  },
];
...
function extractDetailPairs(details: Record<string, unknown>): DetailPair[] {
  ...
  if (details.email)
    pairs.push({ label: "Email", value: String(details.email) });
  ...
  if (details.itemName)
    pairs.push({ label: "Item", value: String(details.itemName) });
  if (details.item_name)
    pairs.push({ label: "Item", value: String(details.item_name) });
  if (details.supplierName)
    pairs.push({ label: "Supplier", value: String(details.supplierName) });
  if (details.recipeName)
    pairs.push({ label: "Recipe", value: String(details.recipeName) });
  if (details.menuItemName)
    pairs.push({ label: "Menu Item", value: String(details.menuItemName) });
  if (details.ingredientName)
    pairs.push({ label: "Ingredient", value: String(details.ingredientName) });
  if (details.poNumber)
    pairs.push({ label: "PO Number", value: String(details.poNumber) });
  if (details.categoryName)
    pairs.push({ label: "Category", value: String(details.categoryName) });
  if (details.tagName)
    pairs.push({ label: "Tag", value: String(details.tagName) });
  if (
    details.previousQuantity !== undefined &&
    details.quantity !== undefined
  ) {
    pairs.push({
      label: "Quantity",
      value: `${details.previousQuantity} \u2192 ${details.quantity}`,
    });
  ...
  if (details.unitCost !== undefined) {
    pairs.push({
      label: "Unit Cost",
      value: `\u00A3${Number(details.unitCost).toFixed(2)}`,
    });
  }
  if (details.sellingPrice !== undefined) {
    pairs.push({
      label: "Selling Price",
      value: `\u00A3${Number(details.sellingPrice).toFixed(2)}`,
    });
  }
  if (details.parLevel !== undefined) {
    pairs.push({
      label: "PAR Level",
      value: details.parLevel === null ? "Cleared" : String(details.parLevel),
    });
  }
  if (details.itemCount)
    pairs.push({ label: "Items", value: `${details.itemCount} items` });
  if (details.itemsImported)
    pairs.push({ label: "Imported", value: `${details.itemsImported} items` });
  if (details.itemsUpdated)
    pairs.push({ label: "Updated", value: `${details.itemsUpdated} items` });
  if (details.itemsReceived)
    pairs.push({ label: "Received", value: `${details.itemsReceived} items` });
  if (details.hoursWorked)
    pairs.push({ label: "Hours Worked", value: `${details.hoursWorked}h` });
  if (details.reason)
    pairs.push({ label: "Reason", value: String(details.reason) });
  if (details.storeName)
    pairs.push({ label: "Store", value: String(details.storeName) });
  if (details.store_name)
    pairs.push({ label: "Store", value: String(details.store_name) });
  if (details.provider)
    pairs.push({ label: "Provider", value: String(details.provider) });
...
function formatCompactDetails(
  ...
  switch (action) {
    ...
    case "shift.clock_out":
      if (details.hoursWorked)
        return `${Number(details.hoursWorked).toFixed(1)}h worked`;
      return "";
    case "shift.create":
    case "shift.update":
    case "shift.delete":
...
function extractBatchChanges(details: Record<string, unknown>): BatchChange[] {
  ...
  return changes.map((change) => {
    const item = String(
      change.item_name || change.itemName || change.name || "Unknown Item",
    );
    const parts: string[] = [];

    if (change.field && change.from !== undefined && change.to !== undefined) {
      parts.push(
        `${humanizeFieldName(String(change.field))}: ${change.from} \u2192 ${change.to}`,
      );
    }
    ...
    if (change.cost_from !== undefined && change.cost_to !== undefined) {
      parts.push(
        `Cost: \u00A3${Number(change.cost_from).toFixed(2)} \u2192 \u00A3${Number(change.cost_to).toFixed(2)}`,
      );
    }
    if (change.par_from !== undefined && change.par_to !== undefined) {
      parts.push(
        `PAR: ${change.par_from ?? "none"} \u2192 ${change.par_to ?? "none"}`,
      );
    }
    ...
    if (parts.length === 0 && typeof change === "object") {
      const entries = Object.entries(change).filter(
        ([k]) =>
          !["item_name", "itemName", "name", "id", "item_id"].includes(k),
      );
        ...
        <tbody>
          {changes.map((c, i) => (
            <tr
              key={i}
              className={
                i < changes.length - 1 ? "border-b border-border/40" : ""
              }
            >
        ...
        <thead>
          <tr className="border-b border-border/60 bg-muted/30">
            {headers.map((h, i) => (
              <th
                key={i}
                className={`py-2 px-3 font-medium text-muted-foreground text-xs uppercase tracking-wider ${i === 0 ? "text-left" : rightAlignLast && i === headers.length - 1 ? "text-right" : "text-left"}`}
              >
                {h}
              </th>
            ))}
          ...
          {rows.map((row, i) => (
            ...
            >
              {row.map((cell, j) => (
                <td
                  key={j}
                  className={`py-2 px-3 ${j === 0 ? "font-medium" : rightAlignLast && j === row.length - 1 ? "text-right tabular-nums" : ""} ${j > 0 ? "text-muted-foreground" : ""}`}
                >
                  {cell}
                </td>
              ))}
        ...
        <tbody>
          {meaningful.map((c, i) => (
            <tr
              key={i}
              className={
                i < meaningful.length - 1 ? "border-b border-border/40" : ""
              }
            >
...
function renderShiftClockIn(d: Record<string, unknown>, log: AuditLog) {
  ...
  let timingText = "";
  let timingClass = "text-emerald-400";
  if (mins !== undefined) {
    if (mins < -1) {
      timingText = `${Math.abs(Math.round(mins))} min early`;
      timingClass = "text-emerald-400";
    } else if (mins > 1) {
      timingText = `${Math.round(mins)} min late`;
      timingClass = "text-amber-400";
    } else {
      timingText = "On time";
      timingClass = "text-emerald-400";
    }
    ...
    <>
      <DetailGrid>
        {has(d.scheduledStart) && has(d.scheduledEnd) && (
          <DetailField
            label="Shift"
            value={`${fmtTime(String(d.scheduledStart))} \u2014 ${fmtTime(String(d.scheduledEnd))}`}
          />
        )}
        {has(d.clockInTime) && (
          <DetailField
            ...
            value={
              <span>
                ...
                {timingText && (
                  <span className={`ml-2 text-xs font-medium ${timingClass}`}>
                    ({timingText})
                  </span>
                )}
              </span>
    ...
    <>
      <DetailGrid>
        ...
        {has(d.hoursWorked) && (
          <DetailField
            label="Worked"
            value={`${Number(d.hoursWorked).toFixed(1)}h`}
          />
        )}
        {has(d.clockInTime) && (
          <DetailField
            label="Clocked In"
            value={fmtTime(String(d.clockInTime))}
          />
        )}
        {has(d.clockOutTime) && (
          <DetailField
            label="Clocked Out"
            value={fmtTime(String(d.clockOutTime))}
          />
        )}
      </DetailGrid>
      <DetailFooter log={log} />
    ...
    <>
      <DetailGrid>
        ...
        {has(d.startTime) && has(d.endTime) && (
          <DetailField
            label="Scheduled"
            value={`${fmtDateTime(String(d.startTime))} \u2014 ${fmtTime(String(d.endTime))}`}
          />
        )}
...
function renderShiftUpdate(d: Record<string, unknown>, log: AuditLog) {
  const prev = d.previousSchedule as
    | { startTime?: string; endTime?: string }
    | undefined;
  const next = d.newSchedule as
    | { startTime?: string; endTime?: string }
    | undefined;
  ...
  return (
    <>
      <DetailGrid>
        {has(d.employeeName) && (
          <DetailField label="Employee" value={String(d.employeeName)} />
        )}
        {prev?.startTime && prev?.endTime && (
          <DetailField
            label="Previous"
            value={`${fmtDateTime(String(prev.startTime))} \u2014 ${fmtTime(String(prev.endTime))}`}
          />
        )}
        {next?.startTime && next?.endTime && (
          <DetailField
            label="Updated to"
            value={`${fmtDateTime(String(next.startTime))} \u2014 ${fmtTime(String(next.endTime))}`}
          />
        )}
      </DetailGrid>
      <DetailFooter log={log} />
    ...
    <>
      <DetailGrid>
        ...
        {has(d.startTime) && (
          <DetailField
            label="Was"
            value={`${fmtDateTime(String(d.startTime))}${has(d.endTime) ? ` \u2014 ${fmtTime(String(d.endTime))}` : ""}`}
          />
        )}
        {has(d.hoursWorked) && (
          <DetailField
            label="Hours"
            value={`${Number(d.hoursWorked).toFixed(1)}h worked`}
          />
        )}
      </DetailGrid>
      <DetailFooter log={log} />
...
function renderClockTimeCorrection(d: Record<string, unknown>, log: AuditLog) {
  const prev = d.previousClockTimes as
    | { clockIn?: string; clockOut?: string }
    | undefined;
  const next = d.newClockTimes as
    | { clockIn?: string; clockOut?: string }
    | undefined;
    ...
    <>
      <DetailGrid>
        ...
        {sched?.start && sched?.end && (
          <DetailField
            label="Shift"
            value={`${fmtTime(String(sched.start))} \u2014 ${fmtTime(String(sched.end))}`}
          />
        )}
      </DetailGrid>
      {(prev || next) && (
        <DetailTable
          ...
          rows={[
            ...(prev?.clockIn || next?.clockIn
              ? [
                  [
                    "Clock In",
                    prev?.clockIn ? fmtTime(prev.clockIn) : "\u2014",
                    next?.clockIn ? fmtTime(next.clockIn) : "\u2014",
                  ],
                ]
              : []),
            ...(prev?.clockOut || next?.clockOut
              ? [
                  [
                    "Clock Out",
                    prev?.clockOut ? fmtTime(prev.clockOut) : "\u2014",
                    next?.clockOut ? fmtTime(next.clockOut) : "\u2014",
                  ],
                ]
              : []),
          ]}
...
function renderInventoryCreate(d: Record<string, unknown>, log: AuditLog) {
  return (
    <>
      <DetailGrid>
        {has(d.itemName) && (
          <DetailField label="Item" value={String(d.itemName)} />
        )}
        {has(d.category) && (
          <DetailField label="Category" value={String(d.category)} />
        )}
...
function renderInventoryItemUpdate(d: Record<string, unknown>, log: AuditLog) {
  ...
  const pairs: DetailPair[] = [];
  if (d.itemName || d.item_name)
    pairs.push({ label: "Item", value: String(d.itemName || d.item_name) });
  if (d.previousQuantity !== undefined && d.quantity !== undefined) {
    pairs.push({
      label: "Quantity",
      value: `${d.previousQuantity} \u2192 ${d.quantity}`,
    });
  }
  if (!fieldChanges?.length) {
    if (d.unitCost !== undefined)
      pairs.push({
        label: "Unit Cost",
        value: `\u00A3${Number(d.unitCost).toFixed(2)}`,
      });
    if (d.parLevel !== undefined)
      pairs.push({
        label: "PAR Level",
        value: d.parLevel === null ? "Cleared" : String(d.parLevel),
      });
    if (
      d.changes &&
      ...
      const fields = Object.keys(d.changes as object).filter(
        (k) => k !== "updated_at",
      );
      if (fields.length > 0)
        pairs.push({ label: "Changed", value: humanizeFieldList(fields) });
    }
  }
  ...
  return (
    <>
      <DetailGrid>
        {pairs.map((p, i) => (
          <DetailField key={i} label={p.label} value={p.value} />
        ))}
      </DetailGrid>
...
function renderBulkImport(d: Record<string, unknown>, log: AuditLog) {
  ...
  return (
    <>
      <DetailGrid>
        {has(d.itemsImported) && (
          <DetailField label="Imported" value={`${d.itemsImported} items`} />
        )}
        {categories && categories.length > 0 && (
          <DetailField label="Categories" value={categories.join(", ")} />
        )}
      </DetailGrid>
      <DetailFooter log={log} />
    </>
...
function renderRecipe(d: Record<string, unknown>, log: AuditLog) {
  ...
  return (
    <>
      <DetailGrid>
        {has(d.recipeName) && (
          <DetailField label="Recipe" value={String(d.recipeName)} />
        )}
        {!fieldChanges?.length &&
          has(d.updatedFields) &&
          Array.isArray(d.updatedFields) && (
            <DetailField
              label="Changed"
              value={humanizeFieldList(d.updatedFields as string[])}
            />
          )}
      </DetailGrid>
      {fieldChanges && fieldChanges.length > 0 && (
...
function renderRecipeIngredient(d: Record<string, unknown>, log: AuditLog) {
  return (
    <>
      <DetailGrid>
        {has(d.ingredientName) && (
          <DetailField label="Ingredient" value={String(d.ingredientName)} />
        )}
        {has(d.quantity) && (
          <DetailField
            label="Quantity"
            value={`${d.quantity}${has(d.unitOfMeasure) ? ` ${d.unitOfMeasure}` : ""}`}
          />
        )}
      </DetailGrid>
      <DetailFooter log={log} />
...
function renderMenuItem(d: Record<string, unknown>, log: AuditLog) {
  ...
  return (
    <>
      <DetailGrid>
        {has(d.menuItemName) && (
          <DetailField label="Menu Item" value={String(d.menuItemName)} />
        )}
        {has(d.sellingPrice) && !fieldChanges?.length && (
          <DetailField
            label="Price"
            value={`\u00A3${Number(d.sellingPrice).toFixed(2)}`}
          />
        )}
        {!fieldChanges?.length &&
          has(d.updatedFields) &&
  ...
  return (
    <>
      <DetailGrid>
        <DetailField
          label="Email"
          value={String(d.email || d.invitedEmail || "")}
        />
        {has(d.role) && <DetailField label="Role" value={String(d.role)} />}
        {has(d.storeName) && (
          <DetailField label="Store" value={String(d.storeName)} />
        )}
      </DetailGrid>
      <DetailFooter log={log} />
    </>
    ...
    <>
      <DetailGrid>
          ...
          <DetailField
            ...
            value={
              <span>
                ...
                {has(d.removedUserEmail) && (
                  <span className="text-muted-foreground font-normal ml-1">
                    ({String(d.removedUserEmail)})
                  </span>
                )}
              </span>
            }
        ...
        {has(d.activeShiftsEnded) && Number(d.activeShiftsEnded) > 0 && (
          <DetailField
            label="Shifts Ended"
            value={`${d.activeShiftsEnded} active shift${Number(d.activeShiftsEnded) !== 1 ? "s" : ""}`}
          />
        )}
      </DetailGrid>
      <DetailFooter log={log} />
    ...
    <>
      <DetailGrid>
        ...
        {has(d.unitCost) && (
          <DetailField
            label="Cost"
            value={`\u00A3${Number(d.unitCost).toFixed(2)}${has(d.currency) ? ` (${d.currency})` : ""}`}
          />
        )}
      </DetailGrid>
      <DetailFooter log={log} />
...
function renderPurchaseOrder(d: Record<string, unknown>, log: AuditLog) {
  return (
    <>
      <DetailGrid>
        {has(d.poNumber) && (
          <DetailField label="PO Number" value={String(d.poNumber)} />
        )}
        {has(d.supplierName) && (
          <DetailField label="Supplier" value={String(d.supplierName)} />
        )}
        {has(d.itemCount) && (
          <DetailField label="Items" value={`${d.itemCount} items`} />
        )}
        {has(d.itemsReceived) && (
          <DetailField label="Received" value={`${d.itemsReceived} items`} />
        )}
        {(has(d.newStatus) || has(d.status)) && (
          <DetailField label="Status" value={String(d.newStatus || d.status)} />
        )}
      </DetailGrid>
      <DetailFooter log={log} />
    </>
    ...
    <>
      <DetailGrid>
        ...
        {has(d.cancelledInvites) && Number(d.cancelledInvites) > 0 && (
          <DetailField
            label="Cancelled"
            value={`${d.cancelledInvites} pending invitations`}
          />
        )}
      </DetailGrid>
      {fieldChanges && fieldChanges.length > 0 && (
...
function renderSettings(d: Record<string, unknown>, log: AuditLog) {
  ...
  if (subAction === "api_key_created") {
    if (d.key_name)
      pairs.push({ label: "Key Name", value: String(d.key_name) });
    if (d.scopes && Array.isArray(d.scopes))
      pairs.push({ label: "Scopes", value: (d.scopes as string[]).join(", ") });
    ...
    if (d.events && Array.isArray(d.events))
      pairs.push({ label: "Events", value: (d.events as string[]).join(", ") });
    ...
    if (d.provider)
      pairs.push({ label: "Provider", value: String(d.provider) });
    ...
    if (d.posItemName)
      pairs.push({ label: "POS Item", value: String(d.posItemName) });
    if (d.quantityPerSale)
      pairs.push({ label: "Qty Per Sale", value: String(d.quantityPerSale) });
  }
      ...
      {has(d.method) && (
        <DetailGrid>
          <DetailField
            label="Method"
            value={
              String(d.method).charAt(0).toUpperCase() +
              String(d.method).slice(1)
            }
          />
        </DetailGrid>
      )}
...
function ExpandedDetails({ log }: { log: AuditLog }) {
  ...
  switch (log.action) {
    case "stock.reception_submit":
    case "stock.reception":
      content = renderStockReception(d, log);
      break;
    case "stock.count_submit":
    case "stock.count":
      content = renderStockCount(d, log);
      break;
    case "waste.submit":
    case "stock.waste_report":
      content = renderWaste(d, log);
      break;
    case "inventory.batch_update":
      content = renderBatchUpdate(d, log);
      break;
    case "inventory.item_create":
    case "inventory.create":
      content = renderInventoryCreate(d, log);
      break;
    case "inventory.item_update":
    case "inventory.update":
      content = renderInventoryItemUpdate(d, log);
      break;
    case "inventory.item_delete":
    case "inventory.delete":
      content = renderInventoryDelete(d, log);
      break;
    case "inventory.bulk_delete":
      content = renderBulkDelete(d, log);
      break;
    case "inventory.bulk_import":
      content = renderBulkImport(d, log);
      break;
    case "inventory.recipe_create":
    case "inventory.recipe_update":
    case "inventory.recipe_delete":
      content = renderRecipe(d, log);
      break;
    case "inventory.recipe_ingredient_add":
    case "inventory.recipe_ingredient_remove":
      content = renderRecipeIngredient(d, log);
      break;
    case "inventory.menu_item_create":
    case "inventory.menu_item_update":
    case "inventory.menu_item_delete":
      content = renderMenuItem(d, log);
      break;
    case "shift.clock_in":
      content = renderShiftClockIn(d, log);
      break;
    case "shift.clock_out":
      content = renderShiftClockOut(d, log);
      break;
    case "shift.create":
      content = renderShiftCreate(d, log);
      break;
    case "shift.update":
      content = renderShiftUpdate(d, log);
      break;
    case "shift.delete":
      content = renderShiftDelete(d, log);
      break;
    case "shift.clock_time_correction":
      content = renderClockTimeCorrection(d, log);
      break;
    case "user.invite":
    case "user.invite_cancel":
    case "user.invite_resend":
      content = renderUserInvite(d, log);
      break;
    case "user.remove_from_store":
    case "user.deactivate":
      content = renderUserRemove(d, log);
      break;
    case "supplier.create":
    case "supplier.update":
    case "supplier.delete":
      content = renderSupplier(d, log);
      break;
    case "supplier.item_add":
    case "supplier.item_update":
    case "supplier.item_remove":
      content = renderSupplierItem(d, log);
      break;
    ...
    case "supplier.po_cancel":
      content = renderPurchaseOrder(d, log);
      break;
    case "store.create":
    case "store.update":
    case "store.delete":
    case "store.deactivate":
      content = renderStore(d, log);
      break;
    ...
    case "settings.pos_connection_delete":
      content = renderSettings(d, log);
      break;
    case "auth.login":
    case "auth.logout":
      content = renderAuth(d, log);
      break;
    default:
...
function ActivityRow({
  ...
  const borderColor =
    CATEGORY_BORDER_COLORS[log.action_category] || "border-l-gray-300";
  const hasDetails = log.details && Object.keys(log.details).length > 0;
  const compactDetail = hasDetails
    ? formatCompactDetails(log.action, log.details)
    : "";
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <Avatar
            name={displayName}
            identifier={log.user_email || displayName}
          />
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            <Badge
              className={`text-[10px] px-1.5 py-0 ${CATEGORY_COLORS[log.action_category] || "bg-gray-100 text-gray-800"} hidden sm:inline-flex`}
            >
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {currentStore?.store?.name} &middot; Complete audit trail for your
            store
          </p>
        </div>
        ...
        <Card>
          <CardContent className="py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed max-w-sm mx-auto">
              No events match your current filters. Try expanding the date range
            ...
            <div className="divide-y divide-border/50">
              {data.logs.map((log, idx) => {
                ...
                const prevDate = prevLog
                  ? format(new Date(prevLog.created_at), "EEEE, MMMM d")
                  : null;
