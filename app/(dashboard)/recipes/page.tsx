...
export default function MenuCostsPage() {
  ...
  const [editingCostItemId, setEditingCostItemId] = useState<string | null>(
    null,
  );
  ...
  const existingCategories = useMemo(
    () => [
      ...new Set(
        menuItems.map((m) => m.category).filter((c): c is string => !!c),
      ),
    ],
    [menuItems],
  );
  ...
  if (!storeId) {
    return (
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="pt-6 text-center text-muted-foreground">
            Select a store to see your menu.
          </CardContent>
        </Card>
      </div>
    );
  ...
  const handleCreateUnifiedItem = async (data: {
    name: string;
    category?: string;
  ...
  const handleAddCostBreakdown = async (
    menuItemId: string,
    menuItemName: string,
  ) => {
    try {
  ...
  const handleDeleteMenuItem = async () => {
    ...
    try {
      ...
      if (menuItem?.recipe_id) {
        try {
          await deleteRecipe(menuItem.recipe_id);
        } catch {
          /* recipe may already be gone */
        }
      }
  ...
  const handleCostsRowClick = (item: {
    id: string;
    name: string;
    has_recipe: boolean;
  }) => {
          ...
          <Card>
            <CardContent className="pt-4">
              ...
              <p className="text-xl font-bold">
                {formatCurrency(recipeDetail.total_cost)}
              </p>
            </CardContent>
          </Card>
          {selectedMenuItem && (
              ...
              <Card>
                <CardContent className="pt-4">
                  ...
                  <p className="text-xl font-bold text-emerald-400">
                    {formatCurrency(
                      selectedMenuItem.selling_price - recipeDetail.total_cost,
                    )}
                  </p>
                </CardContent>
              </Card>
                      ...
                      <div className="flex items-center gap-4 text-sm">
                          ...
                          <div className="font-medium">
                            {editingCostItemId === ing.inventory_item_id ? (
                              <Input
                                ...
                                onBlur={() =>
                                  handleSaveCost(ing.inventory_item_id)
                                }
                                onKeyDown={(e) => {
                                  if (e.key === "Enter")
                                    handleSaveCost(ing.inventory_item_id);
                              ...
                              <button
                                type="button"
                                className={`${(ing.unit_cost ?? 0) === 0 ? "text-amber-400 underline decoration-dashed cursor-pointer" : "cursor-pointer hover:underline"}`}
                                onClick={() => {
                                  setEditingCostItemId(ing.inventory_item_id);
                                  setEditingCostValue(
                                    ing.unit_cost ? String(ing.unit_cost) : "",
                                  );
                                }}
                              >
                                {(ing.unit_cost ?? 0) === 0
                                  ? "Set cost"
                                  : formatCurrency(ing.unit_cost ?? 0)}
                              </button>
                            )}
                          </div>
                        </div>
                        <div>
                          ...
                          <p className="font-medium">
                            {formatCurrency(ing.line_cost ?? 0)}
                          </p>
                        </div>
                      </div>
                    </div>
                        ...
                        <TableRow key={ing.id}>
                          ...
                          <TableCell className="text-right">
                            {editingCostItemId === ing.inventory_item_id ? (
                              ...
                              <button
                                type="button"
                                className={`text-right w-full ${(ing.unit_cost ?? 0) === 0 ? "text-amber-400 underline decoration-dashed cursor-pointer" : "cursor-pointer hover:underline"}`}
                                onClick={() => {
                                  setEditingCostItemId(ing.inventory_item_id);
                                  setEditingCostValue(
                                    ing.unit_cost ? String(ing.unit_cost) : "",
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex flex-col items-center text-center max-w-md mx-auto space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Add the items you sell — burgers, wraps, drinks — and
                  we&apos;ll help you track what they cost to make.
                </p>
              </div>
              <div className="w-full space-y-3 text-left">
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                  <span className="flex items-center justify-center rounded-full bg-card border h-6 w-6 text-xs font-medium shrink-0">
                    1
                  </span>
                  <div>
                    <p className="text-sm font-medium">Add a menu item</p>
                    <p className="text-xs text-muted-foreground">
                      What you sell, which category it&apos;s in, and the price
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Pick ingredients from your inventory
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      We&apos;ll calculate cost, profit, and warn you if costs
                      get too high
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <Card>
                    <CardContent className="pt-4">
                      ...
                      <p className="text-2xl font-bold mt-1">
                        {analysis.summary.total_menu_items}
                      </p>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardContent className="pt-4">
                      ...
                      <p className="text-2xl font-bold mt-1">
                        {analysis.summary.average_food_cost_percentage.toFixed(
                          1,
                        )}
                        %
                      </p>
                  ...
                  <Card>
                    <CardContent className="pt-4">
                      ...
                      <p
                        className={`text-2xl font-bold mt-1 ${analysis.cost_alerts.length > 0 ? "text-amber-400" : ""}`}
                      >
                  ...
                  <Card className="border-amber-500/20">
                    <CardHeader className="pb-3">
                      <CardTitle className="text-sm font-medium flex items-center gap-2">
                        ...
                        Items above target cost (
                        {analysis.summary.target_food_cost_percentage}%)
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      ...
                      {analysis.items.map((item) => (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            ...
                            <Badge
                              variant="secondary"
                              className={RATING_COLORS[item.rating]}
                            >
      ...
      <MenuItemForm
        ...
        onOpenChange={(open) => {
          setShowMenuItemForm(open);
          if (!open) setPrefillCategory(undefined);
        }}
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => {
          if (!open) setDeletingMenuItemId(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure? This will remove the item and its cost breakdown
              from your menu.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
