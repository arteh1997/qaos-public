...
function OnboardContent() {
    ...
    async function validateToken() {
      ...
      try {
        const response = await fetch(
          `/api/users/onboard/validate?token=${token}`,
        );
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-muted-foreground mb-6">
                This invitation link is invalid or has been corrupted. Please
        ...
        <Card className="w-full max-w-md mx-4">
          <CardContent className="pt-6">
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-muted-foreground mb-6">
                This invitation has already been used. If you&apos;ve completed
                your registration, you can log in with your credentials.
              </p>
              <Button onClick={() => router.push("/login")}>Go to Login</Button>
            </div>
          </CardContent>
  ...
  const roleDisplay =
    inviteDetails?.role === "Owner" ? "Co-Owner" : inviteDetails?.role;

  return (
    <div className="min-h-screen flex items-center justify-center bg-background py-12 px-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <div className="mx-auto mb-4 text-4xl">Welcome</div>
          <CardTitle className="text-2xl">Complete Your Account</CardTitle>
          <CardDescription>
            You&apos;ve been invited to join as a <strong>{roleDisplay}</strong>
            {inviteDetails?.storeName && (
              <>
                {" "}
                at <strong>{inviteDetails.storeName}</strong>
              </>
            )}
          </CardDescription>
        </CardHeader>
