                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="font-medium text-sm truncate">
                          {(inv.invoice_number as string) || (inv.file_name as string) || 'Untitled'}
                        </span>
                      ...
                      <div className="flex items-center gap-3 text-xs text-muted-foreground pl-6">
                        {(inv.invoice_date as string) ? <span>{formatDate(inv.invoice_date as string)}</span> : null}
                        {inv.total_amount != null ? (
                          <span className="font-medium text-foreground">
                            £{(inv.total_amount as number).toFixed(2)}
                          </span>
                        ) : null}
                        <span>Uploaded {formatDate(inv.created_at as string)}</span>
                      </div>
                    </div>
                  </div>
                </CardContent>
