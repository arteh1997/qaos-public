...
export default function PortalOrderDetailPage() {
  ...
  const [showConfirm, setShowConfirm] = useState<'acknowledged' | 'shipped' | null>(null)
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(showConfirm === 'acknowledged' ? 'Order acknowledged' : 'Marked as shipped')
        fetchOrder(poId)
      } else {
  ...
  const totalOrdered = items.reduce((sum, i) => sum + (i.quantity_ordered as number || 0), 0)
  const totalReceived = items.reduce((sum, i) => sum + (i.quantity_received as number || 0), 0)
  ...
  return (
    <PortalLayout>
      <Button variant="ghost" size="sm" onClick={() => router.push('/portal/orders')} className="mb-4">
      ...
      <Card>
        <CardHeader className="pb-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <CardTitle className="text-lg">PO #{o.po_number as string}</CardTitle>
              <p className="text-sm text-muted-foreground mt-0.5">
                Ordered {formatDate(o.order_date as string)}
                {(o.expected_delivery_date as string) ? ` — Due ${formatDate(o.expected_delivery_date as string)}` : ''}
              </p>
            </div>
          ...
          {!isTerminal && (
            <div className="flex flex-wrap items-center gap-2 pt-3 border-t mt-4">
              {canAcknowledge && status === 'open' && (
                <Button size="sm" variant="outline" onClick={() => setShowConfirm('acknowledged')}>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-medium mt-0.5">{(o.currency as string) || 'GBP'}</p>
            </div>
          </div>
      ...
      {/* Confirm dialog */}
      <AlertDialog open={!!showConfirm} onOpenChange={(open) => { if (!open) setShowConfirm(null) }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {showConfirm === 'acknowledged' ? 'Acknowledge Order?' : 'Mark as Shipped?'}
            </AlertDialogTitle>
            <AlertDialogDescription>
