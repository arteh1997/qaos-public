...
export default function PortalCatalogPage() {
  ...
  const handleEdit = (id: string, field: keyof Omit<EditedItem, 'id'>, value: number) => {
    setEdits(prev => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.success) {
        const failed = (res.data as Array<{ success: boolean }>).filter(r => !r.success).length
        if (failed > 0) {
  ...
  const filteredItems = items.filter(item => {
    ...
    const inv = (item as Record<string, unknown>).inventory_items as Record<string, unknown> | null
    ...
    <PortalLayout title="Product Catalog">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description={search ? 'No items match your search.' : 'Your product catalog is empty.'}
        />
      ) : (
        <>
          {/* Mobile card view */}
          <div className="sm:hidden space-y-2">
            {filteredItems.map(raw => {
              ...
              const inv = item.inventory_items as Record<string, unknown> | null
              const id = item.id as string
              const currentCost = edits.get(id)?.unit_cost ?? (item.unit_cost as number)
              const currentLead = edits.get(id)?.lead_time_days ?? (item.lead_time_days as number | null)
              const currentMin = edits.get(id)?.min_order_quantity ?? (item.min_order_quantity as number)
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div>
                        ...
                        <Input
                          ...
                          min="0"
                          value={currentCost ?? ''}
                          onChange={e => handleEdit(id, 'unit_cost', parseFloat(e.target.value) || 0)}
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          min="0"
                          value={currentLead ?? ''}
                          onChange={e => handleEdit(id, 'lead_time_days', parseInt(e.target.value) || 0)}
                          className="h-8 text-sm mt-0.5"
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          min="0"
                          value={currentMin ?? ''}
                          onChange={e => handleEdit(id, 'min_order_quantity', parseInt(e.target.value) || 0)}
                          className="h-8 text-sm mt-0.5"
                        />
                      </div>
                    </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right w-28">Lead Time (days)</TableHead>
                    <TableHead className="text-right w-28">Min Order Qty</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredItems.map(raw => {
                    ...
                    return (
                      <TableRow key={id} className={hasEdits ? 'bg-amber-50/50' : ''}>
                        <TableCell>
                          <div>
                            <span className="font-medium">{(inv?.name as string) || 'Unknown'}</span>
                            {(inv?.unit_of_measure as string) ? (
                              <span className="text-xs text-muted-foreground ml-1">
                                ({inv!.unit_of_measure as string})
                              </span>
