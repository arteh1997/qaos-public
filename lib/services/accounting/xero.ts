...
async function xeroFetch(
  ...
  const url = path.startsWith('http') ? path : `${XERO_OAUTH_CONFIG.apiBaseUrl}${path}`
...
export const xeroAdapter: AccountingProviderAdapter = {
  ...
  async createBill(
    ...
    if (!res.ok) {
      ...
      return { success: false, error: `Xero API error: ${res.status} ${errorText}` }
    }
    ...
    if (created?.HasErrors) {
      const errorMessage = created.ValidationErrors
        ?.map((e: { Message: string }) => e.Message)
        .join('; ')
      return { success: false, error: errorMessage || 'Xero validation failed' }
    }
  ...
  async updateBill(
    ...
    const xeroInvoice = { ...mapBillToXeroInvoice(bill), InvoiceID: externalId }
...
export async function getXeroCredentials(
  ...
  return getValidCredentials(connectionId, credentials, xeroAdapter.refreshToken)
}

/**
 * Disconnect from Xero — revoke the token.
 */
export async function revokeXeroToken(credentials: AccountingCredentials): Promise<void> {
  try {
