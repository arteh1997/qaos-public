...
export const quickbooksAdapter: AccountingProviderAdapter = {
  ...
  async findOrCreateContact(
    ...
    if (!createRes.ok) {
      throw new Error(`Failed to create QuickBooks vendor: ${createRes.status}`)
    }
  ...
  async createBill(
    ...
    if (!res.ok) {
      ...
      return { success: false, error: `QuickBooks API error: ${res.status} ${errorText}` }
    }
  ...
  async updateBill(
    ...
    if (!getRes.ok) {
      return { success: false, error: `Failed to fetch bill for update: ${getRes.status}` }
    }
...
export async function getQuickBooksCredentials(
  ...
  return getValidCredentials(connectionId, credentials, quickbooksAdapter.refreshToken)
}

/**
 * Disconnect from QuickBooks — revoke the token.
 */
export async function revokeQuickBooksToken(credentials: AccountingCredentials): Promise<void> {
  try {
