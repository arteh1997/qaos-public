...
export interface AccountingProviderAdapter {
  ...
  // Contacts / Suppliers
  findOrCreateContact(credentials: AccountingCredentials, contact: AccountingContact): Promise<string>

  // Bills (supplier invoices)
  createBill(credentials: AccountingCredentials, bill: AccountingBill): Promise<SyncResult>
  updateBill(credentials: AccountingCredentials, externalId: string, bill: AccountingBill): Promise<SyncResult>

  // Payments
  getBillPaymentStatus(credentials: AccountingCredentials, externalId: string): Promise<string>
}
