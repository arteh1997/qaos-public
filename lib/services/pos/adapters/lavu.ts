...
import { validateLavuSignature } from '../webhook-validators'

/**
 * Lavu iPad POS Adapter
 *
 * iPad-based POS for restaurants and bars.
 * Uses API key authentication (no OAuth flow).
 * API key is stored in the connection's credentials.
 */
export const lavuAdapter: PosProviderAdapter = {
  provider: 'lavu',
  name: 'Lavu',
  authType: 'api_key',
  ...
  validateSignature(payload: string, signature: string, secret: string): boolean {
    return validateLavuSignature(payload, signature, secret)
  },

  normalizeEvent(rawPayload: unknown): PosSaleEvent | null {
    const body = rawPayload as Record<string, unknown>
    const orderId = body.order_id as string || body.orderId as string || body.id as string
    ...
    return {
      ...
      items: items.map(item => ({
        pos_item_id: String(item.item_id || item.id || item.productId || ''),
  ...
  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    ...
    const res = await fetch('https://api.lavu.com/v1/menu/items', {
      headers: { Authorization: `Bearer ${apiKey}`, Accept: 'application/json' },
    })
    if (!res.ok) throw new Error(`Lavu menu fetch failed: ${res.status}`)
