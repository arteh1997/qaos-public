...
import { validateSquirrelSignature } from '../webhook-validators'

/**
 * Squirrel Systems Adapter
 *
 * Hospitality POS for restaurants and hotels. Uses API key authentication.
 * API key is stored in the connection's credentials.
 */
export const squirrelAdapter: PosProviderAdapter = {
  provider: 'squirrel',
  ...
  validateSignature(payload: string, signature: string, secret: string): boolean {
    return validateSquirrelSignature(payload, signature, secret)
  },
  ...
  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    ...
    const res = await fetch('https://api.squirrelsystems.com/v1/menu/items', {
      headers: { Authorization: `Bearer ${apiKey}`, Accept: 'application/json' },
    })
    if (!res.ok) throw new Error(`Squirrel Systems menu fetch failed: ${res.status}`)
