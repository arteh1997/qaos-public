...
import { validateSpeedlineSignature } from '../webhook-validators'

/**
 * Speedline Adapter
 *
 * Speedline Pizza POS — specialized POS for pizza and delivery restaurants.
 * Uses API key authentication.
 */
export const speedlineAdapter: PosProviderAdapter = {
  provider: 'speedline',
  name: 'Speedline',
  authType: 'api_key',
  ...
  validateSignature(payload: string, signature: string, secret: string): boolean {
    return validateSpeedlineSignature(payload, signature, secret)
  },
  ...
  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    ...
    const res = await fetch('https://api.speedlinesolutions.com/v1/menu/items', {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        Accept: 'application/json',
      },
    })
    if (!res.ok) throw new Error(`Speedline menu fetch failed: ${res.status}`)
