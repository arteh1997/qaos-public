...
import { validateDigitalDiningSignature } from '../webhook-validators'

/**
 * Digital Dining Adapter
 *
 * Digital Dining by Menusoft — restaurant POS and management system.
 * Uses API key authentication.
 */
export const digitalDiningAdapter: PosProviderAdapter = {
  ...
  validateSignature(payload: string, signature: string, secret: string): boolean {
    return validateDigitalDiningSignature(payload, signature, secret)
  },
  ...
  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    ...
    const res = await fetch('https://api.digitaldining.com/v1/menu/items', {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        Accept: 'application/json',
      },
    })
    if (!res.ok) throw new Error(`Digital Dining menu fetch failed: ${res.status}`)
