...
import { validateMaitredSignature } from '../webhook-validators'

/**
 * Maitre'D Adapter
 *
 * Maitre'D POS by PayFacto — full-service restaurant management.
 * Uses API key authentication.
 */
export const maitredAdapter: PosProviderAdapter = {
  provider: 'maitred',
  name: "Maitre'D",
  authType: 'api_key',
  ...
  validateSignature(payload: string, signature: string, secret: string): boolean {
    return validateMaitredSignature(payload, signature, secret)
  },
  ...
  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    ...
    const res = await fetch('https://api.maitredpos.com/v1/menu/items', {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        Accept: 'application/json',
      },
    })
    if (!res.ok) throw new Error(`Maitre'D menu fetch failed: ${res.status}`)
