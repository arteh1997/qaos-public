...
import { validateXenialSignature } from '../webhook-validators'

/**
 * Xenial Adapter
 *
 * Global Payments POS for restaurants and quick-service. Uses API key authentication.
 * API key is stored in the connection's credentials.
 */
export const xenialAdapter: PosProviderAdapter = {
  provider: 'xenial',
  name: 'Xenial',
  authType: 'api_key',
  ...
  validateSignature(payload: string, signature: string, secret: string): boolean {
    return validateXenialSignature(payload, signature, secret)
  },
  ...
  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    ...
    const res = await fetch('https://api.xenial.com/v1/menu/items', {
      headers: { Authorization: `Bearer ${apiKey}`, Accept: 'application/json' },
    })
    if (!res.ok) throw new Error(`Xenial menu fetch failed: ${res.status}`)
