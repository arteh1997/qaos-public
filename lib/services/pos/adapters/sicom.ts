...
import { validateSicomSignature } from '../webhook-validators'

/**
 * SICOM Adapter
 *
 * SICOM QSR POS — popular in quick-service restaurant chains.
 * Uses API key authentication.
 */
export const sicomAdapter: PosProviderAdapter = {
  provider: 'sicom',
  name: 'SICOM',
  authType: 'api_key',
  ...
  validateSignature(payload: string, signature: string, secret: string): boolean {
    return validateSicomSignature(payload, signature, secret)
  },
  ...
  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    ...
    const res = await fetch('https://api.sicom.com/v1/menu/items', {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        Accept: 'application/json',
      },
    })
    if (!res.ok) throw new Error(`SICOM menu fetch failed: ${res.status}`)
