...
export const cloverAdapter: PosProviderAdapter = {
  ...
  getAuthUrl(_storeId: string, stateToken: string): string {
    ...
    return `https://sandbox.dev.clover.com/oauth/v2/authorize?${params}`
  },

  async exchangeCode(code: string): Promise<PosOAuthTokens> {
    const res = await fetch(
      `https://sandbox.dev.clover.com/oauth/v2/token?client_id=${getAppId()}&client_secret=${getAppSecret()}&code=${code}`,
    )
  ...
  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    ...
    const res = await fetch(
      `https://api.clover.com/v3/merchants/${merchantId}/items?expand=categories`,
    ...
    return (data.elements || []).map((item: Record<string, unknown>) => ({
      ...
      category: ((item.categories as Record<string, unknown>)?.elements as Record<string, unknown>[])?.[0]?.name as string || undefined,
