...
import { validateFocusPosSignature } from '../webhook-validators'
 ...
 */
export const focusPosAdapter: PosProviderAdapter = {
  provider: 'focus_pos',
  name: 'Focus POS',
  authType: 'api_key',
  ...
  validateSignature(payload: string, signature: string, secret: string): boolean {
    return validateFocusPosSignature(payload, signature, secret)
  },
  ...
  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    ...
    const res = await fetch('https://api.focuspos.com/v1/menu/items', {
      headers: { Authorization: `Bearer ${apiKey}`, Accept: 'application/json' },
    })
    if (!res.ok) throw new Error(`Focus POS menu fetch failed: ${res.status}`)
