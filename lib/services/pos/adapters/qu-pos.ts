...
import { validateQuPosSignature } from '../webhook-validators'

/**
 * Qu POS Adapter
 *
 * Enterprise POS for quick-service restaurants (QSR). Uses API key authentication.
 * API key is stored in the connection's credentials.
 */
export const quPosAdapter: PosProviderAdapter = {
  provider: 'qu_pos',
  name: 'Qu POS',
  authType: 'api_key',
  ...
  validateSignature(payload: string, signature: string, secret: string): boolean {
    return validateQuPosSignature(payload, signature, secret)
  },
  ...
  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    ...
    const res = await fetch('https://api.qupos.com/v1/menu/items', {
      headers: { Authorization: `Bearer ${apiKey}`, Accept: 'application/json' },
    })
    if (!res.ok) throw new Error(`Qu POS menu fetch failed: ${res.status}`)
