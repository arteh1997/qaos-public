...
import { validateAldeloExpressSignature } from '../webhook-validators'

/**
 * Aldelo Express Adapter
 *
 * Cloud-based POS for restaurants. Uses API key authentication.
 * API key is stored in the connection's credentials.
 */
export const aldeloExpressAdapter: PosProviderAdapter = {
  ...
  validateSignature(payload: string, signature: string, secret: string): boolean {
    return validateAldeloExpressSignature(payload, signature, secret)
  },
  ...
  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    ...
    const res = await fetch('https://api.aldeloexpress.com/v1/menu/items', {
      headers: { Authorization: `Bearer ${apiKey}`, Accept: 'application/json' },
    })
    if (!res.ok) throw new Error(`Aldelo Express menu fetch failed: ${res.status}`)
