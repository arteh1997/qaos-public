...
import { validateHarbortouchSignature } from '../webhook-validators'

/**
 * Harbortouch (Shift4) Adapter
 *
 * Harbortouch POS by Shift4 — restaurant and retail POS solution.
 * Uses API key authentication.
 */
export const harbortouchAdapter: PosProviderAdapter = {
  ...
  validateSignature(payload: string, signature: string, secret: string): boolean {
    return validateHarbortouchSignature(payload, signature, secret)
  },
  ...
  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    ...
    const res = await fetch('https://api.harbortouch.com/v1/menu/items', {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        Accept: 'application/json',
      },
    })
    if (!res.ok) throw new Error(`Harbortouch menu fetch failed: ${res.status}`)
