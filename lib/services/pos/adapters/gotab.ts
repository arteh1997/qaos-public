...
import { validateGoTabSignature } from '../webhook-validators'

const getClientId = () => process.env.GOTAB_CLIENT_ID || ''
const getClientSecret = () => process.env.GOTAB_CLIENT_SECRET || ''
const getRedirectUri = () => process.env.GOTAB_REDIRECT_URI || ''

/**
 * GoTab Adapter
 *
 * Order & pay platform for restaurants and breweries.
 * OAuth2 authentication.
 */
export const goTabAdapter: PosProviderAdapter = {
  provider: 'gotab',
  name: 'GoTab',
  authType: 'oauth2',
  ...
  getAuthUrl(_storeId: string, stateToken: string): string {
    ...
    return `https://api.gotab.io/oauth/authorize?${params}`
  },

  async exchangeCode(code: string): Promise<PosOAuthTokens> {
    const res = await fetch('https://api.gotab.io/oauth/token', {
      method: 'POST',
    ...
    if (!res.ok) throw new Error(`GoTab token exchange failed: ${res.status}`)
  ...
  async refreshToken(credentials: Record<string, unknown>): Promise<PosOAuthTokens> {
    ...
    if (!res.ok) throw new Error(`GoTab token refresh failed: ${res.status}`)
    const data = await res.json()
    return {
      access_token: data.access_token,
      refresh_token: data.refresh_token,
  ...
  validateSignature(payload: string, signature: string, secret: string): boolean {
    return validateGoTabSignature(payload, signature, secret)
  },
  ...
  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    const res = await fetch('https://api.gotab.io/v1/menu/items', {
      headers: {
        Authorization: `Bearer ${credentials.access_token}`,
        Accept: 'application/json',
      },
    })
    if (!res.ok) throw new Error(`GoTab menu fetch failed: ${res.status}`)
    const data = await res.json()
