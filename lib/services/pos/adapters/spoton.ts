...
import { validateSpotOnSignature } from '../webhook-validators'

const getClientId = () => process.env.SPOTON_CLIENT_ID || ''
const getClientSecret = () => process.env.SPOTON_CLIENT_SECRET || ''
const getRedirectUri = () => process.env.SPOTON_REDIRECT_URI || ''

/**
 * SpotOn Adapter
 *
 * Popular North American restaurant POS with strong online ordering.
 * OAuth2 authentication.
 */
export const spotOnAdapter: PosProviderAdapter = {
  provider: 'spoton',
  name: 'SpotOn',
  authType: 'oauth2',
  ...
  getAuthUrl(_storeId: string, stateToken: string): string {
    ...
    return `https://api.spoton.com/oauth/authorize?${params}`
  },

  async exchangeCode(code: string): Promise<PosOAuthTokens> {
    const res = await fetch('https://api.spoton.com/oauth/token', {
      method: 'POST',
    ...
    if (!res.ok) throw new Error(`SpotOn token exchange failed: ${res.status}`)
  ...
  validateSignature(payload: string, signature: string, secret: string): boolean {
    return validateSpotOnSignature(payload, signature, secret)
  },

  normalizeEvent(rawPayload: unknown): PosSaleEvent | null {
    const body = rawPayload as Record<string, unknown>
    const orderId = body.orderId as string || body.id as string
    ...
    return {
      ...
      items: items.map(item => ({
        pos_item_id: (item.itemId as string) || (item.id as string) || '',
      ...
      currency: 'USD',
      occurred_at: (body.createdAt as string) || new Date().toISOString(),
    }
  },

  async fetchMenuItems(credentials: Record<string, unknown>): Promise<PosMenuItem[]> {
    const res = await fetch('https://api.spoton.com/v1/menu/items', {
      headers: {
        Authorization: `Bearer ${credentials.access_token}`,
        Accept: 'application/json',
      },
    })
    if (!res.ok) throw new Error(`SpotOn menu fetch failed: ${res.status}`)
    const data = await res.json()
