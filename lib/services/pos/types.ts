...
export interface PosProviderAdapter {
  ...
  // Sales sync — pull recent orders from POS API
  syncSales?(
    credentials: Record<string, unknown>,
    since?: string,
  ): Promise<PosSaleEvent[]>;

  // Connection validation — verify API credentials are valid
  validateConnection?(credentials: Record<string, unknown>): Promise<boolean>;
}
