...
import { rateLimit, getRateLimitHeaders, RATE_LIMITS, RateLimitConfig } from '@/lib/rate-limit'
...
describe('Rate Limiter', () => {
  ...
  describe('rateLimit function', () => {
    ...
    it('should track different identifiers separately', () => {
      ...
      expect(rateLimit(identifier1, config).success).toBe(false)

      // identifier2 should still be allowed
      expect(rateLimit(identifier2, config).success).toBe(true)
    })
    ...
    it('should handle edge case of exactly hitting the limit', () => {
      ...
      const lastAllowed = rateLimit(identifier, config)
  ...
  describe('Edge Cases', () => {
    it('should handle very short window durations', () => {
      ...
      rateLimit(identifier, shortConfig)
      rateLimit(identifier, shortConfig)

      // Should be blocked
      expect(rateLimit(identifier, shortConfig).success).toBe(false)

      // After short window, should reset
      vi.advanceTimersByTime(101)
      expect(rateLimit(identifier, shortConfig).success).toBe(true)
    })
    ...
    it('should handle limit of 1', () => {
      ...
      const first = rateLimit(identifier, oneConfig)
      expect(first.success).toBe(true)
      expect(first.remaining).toBe(0)

      const second = rateLimit(identifier, oneConfig)
