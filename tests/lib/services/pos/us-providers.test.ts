...
describe('validateParBrinkSignature', () => {
  it('should accept a valid hex signature', () => {
    ...
    expect(validateParBrinkSignature(TEST_PAYLOAD, sig, TEST_SECRET)).toBe(true)
  })
  ...
  it('should return false for an invalid signature (same-length hex)', () => {
    ...
    expect(validateParBrinkSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })
  ...
  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateParBrinkSignature(TEST_PAYLOAD, sig, 'wrong_secret')).toBe(false)
  })
...
describe('validateHeartlandSignature', () => {
  it('should accept a valid hex signature', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateHeartlandSignature(TEST_PAYLOAD, sig, TEST_SECRET)).toBe(true)
  })

  it('should return false for an invalid signature (same-length hex)', () => {
    const wrongSig = hmacHex('different-payload-for-heartland', TEST_SECRET)
    expect(validateHeartlandSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })
  ...
  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateHeartlandSignature(TEST_PAYLOAD, sig, 'heartland_wrong')).toBe(false)
  })
...
describe('validateHungerRushSignature', () => {
  it('should accept a valid hex signature', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateHungerRushSignature(TEST_PAYLOAD, sig, TEST_SECRET)).toBe(true)
  })

  it('should return false for an invalid signature (same-length hex)', () => {
    const wrongSig = hmacHex('different-payload-for-hungerrush', TEST_SECRET)
    expect(validateHungerRushSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })
  ...
  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateHungerRushSignature(TEST_PAYLOAD, sig, 'hungerrush_bad')).toBe(false)
  })
...
describe('validateCakeSignature', () => {
  ...
  it('should return false for an invalid signature (same-length hex)', () => {
    ...
    expect(validateCakeSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })
  ...
  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateCakeSignature(TEST_PAYLOAD, sig, 'cake_expired_key')).toBe(false)
  })
...
describe('validateLavuSignature', () => {
  ...
  it('should return false for an invalid signature (same-length hex)', () => {
    ...
    expect(validateLavuSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })
  ...
  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateLavuSignature(TEST_PAYLOAD, sig, 'lavu_wrong_key')).toBe(false)
  })
...
describe('validateFocusPosSignature', () => {
  it('should accept a valid hex signature', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateFocusPosSignature(TEST_PAYLOAD, sig, TEST_SECRET)).toBe(true)
  })

  it('should return false for an invalid signature (same-length hex)', () => {
    const wrongSig = hmacHex('different-payload-for-focus-pos', TEST_SECRET)
    expect(validateFocusPosSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })
  ...
  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateFocusPosSignature(TEST_PAYLOAD, sig, 'focus_leaked')).toBe(false)
  })
...
describe('validateShopifyPosSignature', () => {
  it('should accept a valid base64 signature', () => {
    ...
    expect(validateShopifyPosSignature(TEST_PAYLOAD, sig, TEST_SECRET)).toBe(true)
  })

  it('should return false for an invalid signature (same-length base64)', () => {
    const wrongSig = hmacBase64('different-payload-for-shopify-pos', TEST_SECRET)
    expect(validateShopifyPosSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })
  ...
  it('should return false when a wrong secret is used', () => {
    const sig = hmacBase64(TEST_PAYLOAD, TEST_SECRET)
    expect(validateShopifyPosSignature(TEST_PAYLOAD, sig, 'shopify_wrong')).toBe(false)
  })
...
describe('validateAldeloExpressSignature', () => {
  it('should accept a valid hex signature', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateAldeloExpressSignature(TEST_PAYLOAD, sig, TEST_SECRET)).toBe(true)
  })

  it('should return false for an invalid signature (same-length hex)', () => {
    const wrongSig = hmacHex('different-payload-for-aldelo-express', TEST_SECRET)
    expect(validateAldeloExpressSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })

  it('should return false for a tampered payload', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    const tampered = TEST_PAYLOAD.replace('2499', '0')
    expect(validateAldeloExpressSignature(tampered, sig, TEST_SECRET)).toBe(false)
  })

  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateAldeloExpressSignature(TEST_PAYLOAD, sig, 'aldelo_old_secret')).toBe(false)
  })
...
describe('validateSquirrelSignature', () => {
  it('should accept a valid hex signature', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateSquirrelSignature(TEST_PAYLOAD, sig, TEST_SECRET)).toBe(true)
  })

  it('should return false for an invalid signature (same-length hex)', () => {
    const wrongSig = hmacHex('different-payload-for-squirrel', TEST_SECRET)
    expect(validateSquirrelSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })

  it('should return false for a tampered payload', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    const tampered = TEST_PAYLOAD.replace('payment.completed', 'payment.cancelled')
    expect(validateSquirrelSignature(tampered, sig, TEST_SECRET)).toBe(false)
  })

  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateSquirrelSignature(TEST_PAYLOAD, sig, 'squirrel_rotated')).toBe(false)
  })
...
describe('validateGoTabSignature', () => {
  ...
  it('should return false for an invalid signature (same-length hex)', () => {
    ...
    expect(validateGoTabSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })
  ...
  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateGoTabSignature(TEST_PAYLOAD, sig, 'gotab_compromised')).toBe(false)
  })
...
describe('validateXenialSignature', () => {
  ...
  it('should return false for an invalid signature (same-length hex)', () => {
    ...
    expect(validateXenialSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })
  ...
  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateXenialSignature(TEST_PAYLOAD, sig, 'xenial_bad_key')).toBe(false)
  })
...
describe('validateQuPosSignature', () => {
  ...
  it('should return false for an invalid signature (same-length hex)', () => {
    ...
    expect(validateQuPosSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })
  ...
  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateQuPosSignature(TEST_PAYLOAD, sig, 'qu_pos_expired')).toBe(false)
  })
...
describe('validateFuturePosSignature', () => {
  it('should accept a valid hex signature', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateFuturePosSignature(TEST_PAYLOAD, sig, TEST_SECRET)).toBe(true)
  })

  it('should return false for an invalid signature (same-length hex)', () => {
    const wrongSig = hmacHex('different-payload-for-future-pos', TEST_SECRET)
    expect(validateFuturePosSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })
  ...
  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateFuturePosSignature(TEST_PAYLOAD, sig, 'future_pos_wrong')).toBe(false)
  })
...
describe('validateUpserveSignature', () => {
  ...
  it('should return false for an invalid signature (same-length hex)', () => {
    ...
    expect(validateUpserveSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })

  it('should return false for a tampered payload', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    const tampered = TEST_PAYLOAD.replace('payment.completed', 'payment.reversed')
    expect(validateUpserveSignature(tampered, sig, TEST_SECRET)).toBe(false)
  })

  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateUpserveSignature(TEST_PAYLOAD, sig, 'upserve_old_key')).toBe(false)
  })
...
describe('validateSicomSignature', () => {
  ...
  it('should return false for an invalid signature (same-length hex)', () => {
    ...
    expect(validateSicomSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })
  ...
  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateSicomSignature(TEST_PAYLOAD, sig, 'sicom_leaked_secret')).toBe(false)
  })
...
describe('validatePosiTouchSignature', () => {
  it('should accept a valid hex signature', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validatePosiTouchSignature(TEST_PAYLOAD, sig, TEST_SECRET)).toBe(true)
  })

  it('should return false for an invalid signature (same-length hex)', () => {
    const wrongSig = hmacHex('different-payload-for-positouch', TEST_SECRET)
    expect(validatePosiTouchSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })
  ...
  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validatePosiTouchSignature(TEST_PAYLOAD, sig, 'positouch_wrong')).toBe(false)
  })
...
describe('validateHarbortouchSignature', () => {
  it('should accept a valid hex signature', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateHarbortouchSignature(TEST_PAYLOAD, sig, TEST_SECRET)).toBe(true)
  })

  it('should return false for an invalid signature (same-length hex)', () => {
    const wrongSig = hmacHex('different-payload-for-harbortouch', TEST_SECRET)
    expect(validateHarbortouchSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })

  it('should return false for a tampered payload', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    const tampered = TEST_PAYLOAD.replace('merchant_id', 'bad_merchant')
    expect(validateHarbortouchSignature(tampered, sig, TEST_SECRET)).toBe(false)
  })

  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateHarbortouchSignature(TEST_PAYLOAD, sig, 'harbor_expired')).toBe(false)
  })
...
describe('validateDigitalDiningSignature', () => {
  it('should accept a valid hex signature', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateDigitalDiningSignature(TEST_PAYLOAD, sig, TEST_SECRET)).toBe(true)
  })

  it('should return false for an invalid signature (same-length hex)', () => {
    const wrongSig = hmacHex('different-payload-for-digital-dining', TEST_SECRET)
    expect(validateDigitalDiningSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })

  it('should return false for a tampered payload', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    const tampered = TEST_PAYLOAD.replace('2499', '5000')
    expect(validateDigitalDiningSignature(tampered, sig, TEST_SECRET)).toBe(false)
  })

  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateDigitalDiningSignature(TEST_PAYLOAD, sig, 'dd_bad_secret')).toBe(false)
  })
...
describe('validateMaitredSignature', () => {
  ...
  it('should return false for an invalid signature (same-length hex)', () => {
    ...
    expect(validateMaitredSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })

  it('should return false for a tampered payload', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    const tampered = TEST_PAYLOAD.replace('payment.completed', 'payment.disputed')
    expect(validateMaitredSignature(tampered, sig, TEST_SECRET)).toBe(false)
  })

  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateMaitredSignature(TEST_PAYLOAD, sig, 'maitred_revoked')).toBe(false)
  })
...
describe('validateSpeedlineSignature', () => {
  it('should accept a valid hex signature', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateSpeedlineSignature(TEST_PAYLOAD, sig, TEST_SECRET)).toBe(true)
  })

  it('should return false for an invalid signature (same-length hex)', () => {
    const wrongSig = hmacHex('different-payload-for-speedline', TEST_SECRET)
    expect(validateSpeedlineSignature(TEST_PAYLOAD, wrongSig, TEST_SECRET)).toBe(false)
  })

  it('should return false for a tampered payload', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    const tampered = TEST_PAYLOAD.replace('store_abc', 'store_speedline_tamper')
    expect(validateSpeedlineSignature(tampered, sig, TEST_SECRET)).toBe(false)
  })

  it('should return false when a wrong secret is used', () => {
    const sig = hmacHex(TEST_PAYLOAD, TEST_SECRET)
    expect(validateSpeedlineSignature(TEST_PAYLOAD, sig, 'speedline_old_key')).toBe(false)
  })
...
describe('heartlandAdapter.normalizeEvent', () => {
  it('should normalize a valid sale event', () => {
    const result = heartlandAdapter.normalizeEvent({
      ...
      items: [
        { productId: 'prod_1', name: 'Steak', quantity: 1, price: 2499 },
      ],
      total: 2499,
      createdAt: '2026-01-15T12:00:00Z',
    })
...
describe('hungerRushAdapter.normalizeEvent', () => {
  it('should normalize a valid sale event', () => {
    const result = hungerRushAdapter.normalizeEvent({
      orderId: 'HR-001',
      orderItems: [
        { itemId: 'pizza_1', itemName: 'Pepperoni Pizza', qty: 2, unitPrice: 1499 },
      ],
      total: 2998,
      createdAt: '2026-01-15T12:00:00Z',
    })
...
describe('shopifyPosAdapter.normalizeEvent', () => {
  it('should normalize a valid sale event', () => {
    const result = shopifyPosAdapter.normalizeEvent({
      id: 'SH-001',
      line_items: [
        { product_id: 'sp_1', title: 'Craft Beer', quantity: 4, price: 8.50 },
      ],
      total_price: 34.00,
      currency: 'USD',
    ...
    expect(result!.items[0].unit_price).toBe(8.50)
    expect(result!.total_amount).toBe(34.00)
    expect(result!.currency).toBe('USD')
  })
...
describe('xenialAdapter.normalizeEvent', () => {
  it('should normalize a valid sale event', () => {
    const result = xenialAdapter.normalizeEvent({
      orderId: 'XN-001',
      items: [
        { itemId: 'xn_1', name: 'Combo Meal', quantity: 2, price: 1199 },
      ],
      total: 2398,
      createdAt: '2026-01-15T12:00:00Z',
    })
...
describe('posiTouchAdapter.normalizeEvent', () => {
  it('should normalize a valid sale event', () => {
    const result = posiTouchAdapter.normalizeEvent({
      orderId: 'PT-001',
      items: [
        { itemId: 'pt_1', name: 'Prime Rib', quantity: 1, price: 3999 },
      ],
      total: 3999,
      createdAt: '2026-01-15T12:00:00Z',
    })
...
describe('harbortouchAdapter.normalizeEvent', () => {
  it('should normalize a valid sale event', () => {
    const result = harbortouchAdapter.normalizeEvent({
      orderId: 'HT-001',
      items: [
        { itemId: 'ht_1', name: 'Fish Tacos', quantity: 2, price: 1399 },
      ],
      total: 2798,
      createdAt: '2026-01-15T12:00:00Z',
    })
...
describe('US POS adapter metadata', () => {
  const adapters = [
    ...
    { adapter: heartlandAdapter, provider: 'heartland', name: 'Heartland Restaurant' },
    ...
    { adapter: shopifyPosAdapter, provider: 'shopify_pos', name: 'Shopify POS' },
    { adapter: aldeloExpressAdapter, provider: 'aldelo_express', name: 'Aldelo Express' },
    { adapter: squirrelAdapter, provider: 'squirrel', name: 'Squirrel Systems' },
    ...
    { adapter: harbortouchAdapter, provider: 'harbortouch', name: 'Harbortouch (Shift4)' },
    { adapter: digitalDiningAdapter, provider: 'digital_dining', name: 'Digital Dining' },
