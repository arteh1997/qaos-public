...
function createMockSupabase(data: Record<string, unknown[]>) {
  const createChain = (table: string) => {
    ...
    const chain: Record<string, unknown> = {
      ...
      then: (resolve: (v: unknown) => void) => resolve({ data: filteredData, error: null }),
    }
...
describe('generateFoodCostReport', () => {
  ...
  it('should calculate theoretical cost from recipe ingredients', async () => {
    const supabase = createMockSupabase({
      menu_items: [
        {
          ...
          selling_price: 12.99,
          recipe_id: 'r-1',
          recipe: { id: 'r-1', name: 'Chicken Burger Recipe', yield_quantity: 1 },
        },
      ],
      recipe_ingredients: [
      ...
      store_inventory: [
        ...
        { inventory_item_id: 'inv-2', unit_cost: 0.50 },   // £0.50/bun
      ],
      inventory_items: [
  ...
  it('should calculate waste cost from waste log', async () => {
    const supabase = createMockSupabase({
      ...
      stock_history: [],
      waste_log: [
        { inventory_item_id: 'inv-1', estimated_cost: 15.50 },
  ...
  it('should calculate actual COGS from stock history Sale events', async () => {
    const supabase = createMockSupabase({
      ...
      store_inventory: [
        { inventory_item_id: 'inv-1', unit_cost: 5.00 },
      ],
      waste_log: [],
    })
