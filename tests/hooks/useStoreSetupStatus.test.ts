...
function mockAllFetches(opts: {
  ...
  mockSupabaseFetch
    ...
    .mockResolvedValueOnce({
      data: opts.storeError ? null : [storeData],
...
describe("useStoreSetupStatus", () => {
  ...
  it("should mark inventory step as complete when count > 0", async () => {
    ...
    const inventoryStep = result.current.status.steps.find(
      (s) => s.id === "inventory",
    );
  ...
  it("should mark suppliers step as complete when has suppliers", async () => {
    ...
    const suppliersStep = result.current.status.steps.find(
      (s) => s.id === "suppliers",
    );
