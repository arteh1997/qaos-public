...
const mockOnConnectivityChange = vi.fn(() => vi.fn()) // returns cleanup
const mockSyncPendingOperations = vi.fn()
const mockGetPendingCount = vi.fn(() => Promise.resolve(0))
const mockQueueOperation = vi.fn(() => Promise.resolve(1))

vi.mock('@/lib/offline/sync', () => ({
  isOnline: () => mockIsOnline(),
  onConnectivityChange: (cb: (online: boolean) => void) => mockOnConnectivityChange(cb),
  syncPendingOperations: (csrfFetch: typeof fetch) => mockSyncPendingOperations(csrfFetch),
}))

vi.mock('@/lib/offline/db', () => ({
  getPendingCount: () => mockGetPendingCount(),
  queueOperation: (type: string, storeId: string, data: unknown) => mockQueueOperation(type, storeId, data),
}))
...
describe('useOfflineSync', () => {
  beforeEach(() => {
    ...
    mockSyncPendingOperations.mockResolvedValue({ synced: 0, failed: 0, errors: [] })
  })
  ...
  it('should queue an offline operation', async () => {
    ...
    expect(mockQueueOperation).toHaveBeenCalledWith('stock_count', 'store-123', {
      items: [{ inventory_item_id: 'item-1', quantity: 10 }],
    })
  })
  ...
  it('should return sync result from syncNow', async () => {
    ...
    let syncResult: { synced: number; failed: number; errors: string[] } | undefined
    await act(async () => {
  ...
  it('should prevent concurrent syncs', async () => {
    ...
    act(() => {
      result.current.syncNow().then(() => { firstSyncDone = true })
    })

    // Try second sync while first is in progress
    let secondResult: { synced: number; failed: number; errors: string[] } | undefined
    await act(async () => {
  ...
  it('should refresh pending count after sync', async () => {
    ...
    mockSyncPendingOperations.mockResolvedValue({ synced: 3, failed: 0, errors: [] })
  ...
  it('should refresh pending count after queueing operation', async () => {
    ...
    await act(async () => {
      await result.current.queueOfflineOperation('waste_report', 'store-1', { items: [] })
    })
