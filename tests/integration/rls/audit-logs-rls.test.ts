...
describe.skipIf(!hasRlsCredentials)("RLS: audit_logs", () => {
  ...
  describe("Store Isolation", () => {
    it("should allow Owner A to see only Store A logs", async () => {
      ...
      const storeIds = data
        .map((log: { store_id: string | null }) => log.store_id)
        .filter(Boolean);
  ...
  describe("Immutability", () => {
    ...
    it("should allow service role to insert audit logs", async () => {
      ...
      // Clean up
      await adminAny.from("audit_logs").delete().eq("id", data.id);
    });
  });
  ...
  describe("Multi-Store Users", () => {
    it("should allow users with multiple stores to see logs from all their stores", async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const clientAny = client as any;
      await clientAny.from("store_users").insert({
        store_id: storeB.id,
      ...
      // Should see logs from both stores
      const storeIds = [
        ...new Set(data.map((log: { store_id: string }) => log.store_id)),
      ];
      expect(storeIds).toContain(storeA.id);
      expect(storeIds).toContain(storeB.id);

      // Clean up
      await adminAny.from("audit_logs").delete().eq("id", multiLog.id);
