...
describe('Menu Analysis API', () => {
  ...
  describe('GET /api/stores/[storeId]/menu-analysis', () => {
    it('should return 401 when not authenticated', async () => {
      ...
      const { GET } = await import('@/app/api/stores/[storeId]/menu-analysis/route')
      const request = createMockRequest('GET', `/api/stores/${STORE_UUID}/menu-analysis`)
    ...
    it('should return analysis with menu items and recipes', async () => {
      ...
      const menuItemsQuery = createChainableMock({
        data: [
          {
            id: 'mi-1', name: 'Margherita Pizza', category: 'Pizza',
            selling_price: 15, recipe_id: 'recipe-1', is_active: true,
            recipe: { id: 'recipe-1', name: 'Margherita Recipe', yield_quantity: 2 },
          },
          {
            id: 'mi-2', name: 'Water', category: 'Drinks',
            selling_price: 3, recipe_id: null, is_active: true,
            recipe: null,
          },
        ],
        error: null,
      ...
      const pizzaItem = data.data.items.find((i: { name: string }) => i.name === 'Margherita Pizza')
      ...
      const waterItem = data.data.items.find((i: { name: string }) => i.name === 'Water')
    ...
    it('should respect custom target food cost', async () => {
      ...
      const { GET } = await import('@/app/api/stores/[storeId]/menu-analysis/route')
      const request = createMockRequest('GET', `/api/stores/${STORE_UUID}/menu-analysis`, undefined, {
        targetFoodCost: '25',
      })
      const response = await GET(request, { params: Promise.resolve({ storeId: STORE_UUID }) })
      const data = await response.json()
