...
describe('HACCP Corrective Actions API', () => {
  ...
  describe('GET /api/stores/[storeId]/haccp/corrective-actions', () => {
    it('should return 401 when not authenticated', async () => {
      ...
      const { GET } = await import('@/app/api/stores/[storeId]/haccp/corrective-actions/route')
      const request = createMockRequest('GET', `/api/stores/${STORE_UUID}/haccp/corrective-actions`)
    ...
    it('should return corrective actions list for authenticated user', async () => {
      ...
      expect(data.data[0].description).toBe('Fridge temperature above safe range')
    })
    ...
    it('should filter unresolved_only', async () => {
      ...
      const { GET } = await import('@/app/api/stores/[storeId]/haccp/corrective-actions/route')
      const request = createMockRequest('GET', `/api/stores/${STORE_UUID}/haccp/corrective-actions`, undefined, {
  ...
  describe('POST /api/stores/[storeId]/haccp/corrective-actions', () => {
    it('should return 401 when not authenticated', async () => {
      mockSupabaseClient.auth.getUser.mockResolvedValue({ data: { user: null }, error: null })
      const { POST } = await import('@/app/api/stores/[storeId]/haccp/corrective-actions/route')
      const request = createMockRequest('POST', `/api/stores/${STORE_UUID}/haccp/corrective-actions`, {
        description: 'Fridge temperature above safe range',
        check_id: null,
        temp_log_id: null,
      })
  ...
  describe('PUT /api/stores/[storeId]/haccp/corrective-actions/[actionId]', () => {
    it('should return 401 when not authenticated', async () => {
      mockSupabaseClient.auth.getUser.mockResolvedValue({ data: { user: null }, error: null })
      const { PUT } = await import('@/app/api/stores/[storeId]/haccp/corrective-actions/[actionId]/route')
      const request = createMockRequest('PUT', `/api/stores/${STORE_UUID}/haccp/corrective-actions/${ACTION_UUID}`, {
        action_taken: 'Moved items to backup fridge and called repair service',
      })
      const response = await PUT(request, { params: Promise.resolve({ storeId: STORE_UUID, actionId: ACTION_UUID }) })
      expect(response.status).toBe(401)
    })
    ...
    it('should resolve action successfully for Owner', async () => {
      ...
      expect(response.status).toBe(200)
      expect(data.success).toBe(true)
      expect(data.data.action_taken).toBe('Moved items to backup fridge and called repair service')
