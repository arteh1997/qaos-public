...
describe("Purchase Orders API", () => {
  ...
  describe("GET /api/stores/[storeId]/purchase-orders", () => {
    it("should return 401 when not authenticated", async () => {
      ...
      const { GET } =
        await import("@/app/api/stores/[storeId]/purchase-orders/route");
      const request = createMockRequest(
        "GET",
        `/api/stores/${STORE_UUID}/purchase-orders`,
      );
        ...
        data: [
          {
            ...
            status: "open",
            total_amount: 250,
            supplier: { id: SUPPLIER_UUID, name: "Fresh Foods" },
          },
        ],
        error: null,
        count: 1,
  ...
  describe("POST /api/stores/[storeId]/purchase-orders", () => {
    it("should create purchase order for Owner", async () => {
      ...
      const poItemsInsertQuery = createChainableMock({
        data: null,
        error: null,
      });
  ...
  describe("PUT /api/stores/[storeId]/purchase-orders/[poId]", () => {
    it("should update PO status for Owner", async () => {
      ...
      const { PUT } =
        await import("@/app/api/stores/[storeId]/purchase-orders/[poId]/route");
      const request = createMockRequest(
        "PUT",
        `/api/stores/${STORE_UUID}/purchase-orders/${PO_UUID}`,
        {
          status: "awaiting_delivery",
        },
      );
      const response = await PUT(request, {
        params: Promise.resolve({ storeId: STORE_UUID, poId: PO_UUID }),
      });
      const data = await response.json();
