...
describe('HACCP Checks API', () => {
  ...
  describe('GET /api/stores/[storeId]/haccp/checks', () => {
    it('should return 401 when not authenticated', async () => {
      ...
      const { GET } = await import('@/app/api/stores/[storeId]/haccp/checks/route')

      const request = createMockRequest('GET', `/api/stores/${STORE_UUID}/haccp/checks`)
        ...
        data: [
          {
            ...
            notes: 'All good',
            items: [
              { template_item_id: '1', label: 'Fridge temp OK?', value: true, passed: true },
            ],
            template: null,
          },
          {
            ...
            items: [
              { template_item_id: '1', label: 'Fridge temp OK?', value: false, passed: false },
            ],
    ...
    it('should apply status filter', async () => {
      ...
      const request = createMockRequest('GET', `/api/stores/${STORE_UUID}/haccp/checks`, undefined, {
        status: 'fail',
      })
      const response = await GET(request, { params: Promise.resolve({ storeId: STORE_UUID }) })
      const data = await response.json()
  ...
  describe('POST /api/stores/[storeId]/haccp/checks', () => {
    const validCheckBody = {
      ...
      items: [
        { template_item_id: '1', label: 'Fridge temp OK?', value: true, passed: true },
        { template_item_id: '2', label: 'Hands washed?', value: true, passed: true },
      ],
      status: 'pass',
      notes: 'All good',
    }
    ...
    it('should return 401 when not authenticated', async () => {
      ...
      const { POST } = await import('@/app/api/stores/[storeId]/haccp/checks/route')

      const request = createMockRequest('POST', `/api/stores/${STORE_UUID}/haccp/checks`, validCheckBody)
    ...
    it('should return 400 for invalid data with missing status', async () => {
      ...
      const request = createMockRequest('POST', `/api/stores/${STORE_UUID}/haccp/checks`, {
        template_id: null,
        items: [
