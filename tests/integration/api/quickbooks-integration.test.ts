...
describe("QuickBooks Accounting Integration API", () => {
  ...
  describe("GET /api/integrations/quickbooks/auth", () => {
    it("should redirect to QuickBooks OAuth URL", async () => {
      ...
      const { GET } =
        await import("@/app/api/integrations/quickbooks/auth/route");
  ...
  describe("GET /api/integrations/quickbooks/callback", () => {
    it("should redirect with error when error param is present", async () => {
      const { GET } =
        await import("@/app/api/integrations/quickbooks/callback/route");
      const request = createMockRequest(
        "GET",
    ...
    it("should redirect with error when state token has expired", async () => {
      ...
      const request = createMockRequest(
        ...
        {
          code: "auth-code",
          state: sampleExpiredOauthState.state_token,
          realmId: REALM_ID,
        },
      );
      const response = await GET(request);
    ...
    it("should successfully exchange code for tokens and store connection", async () => {
      ...
      const upsertConnectionQuery = createChainableMock({
        data: null,
        error: null,
      });
      ...
      const request = createMockRequest(
        ...
        {
          code: "auth-code-123",
          state: sampleOauthState.state_token,
          realmId: REALM_ID,
        },
      );
      const response = await GET(request);
    ...
    it("should mark state token as used after successful validation", async () => {
      ...
      expect(updateStateQuery.eq).toHaveBeenCalledWith(
        "id",
        sampleOauthState.id,
      );
    });
  ...
  describe("POST /api/integrations/quickbooks/disconnect", () => {
    it("should deactivate connection and revoke token", async () => {
      ...
      const { POST } =
        await import("@/app/api/integrations/quickbooks/disconnect/route");
      const request = createMockRequest(
        "POST",
      ...
      expect(mockRevokeQuickBooksToken).toHaveBeenCalledWith(
        sampleConnection.credentials,
      );
    });
