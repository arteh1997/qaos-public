...
describe("POS API Integration Tests", () => {
  ...
  describe("POST /api/pos/webhook/[connectionId]", () => {
    it("should return 404 for invalid connection", async () => {
      ...
      const request = createMockRequest(
        "POST",
        "/api/pos/webhook/bad-conn-id",
        {
          event_id: "evt-1",
          items: [
            { pos_item_id: "item-1", pos_item_name: "Burger", quantity: 1 },
          ],
        },
      );
      const response = await POST(request, {
        params: Promise.resolve({ connectionId: "bad-conn-id" }),
      });
      const data = await response.json();
        ...
        items: [
          {
            ...
            pos_item_name: "Classic Burger",
            quantity: 2,
            unit_price: 12.99,
          },
          {
            pos_item_id: "fries-1",
            pos_item_name: "Large Fries",
            quantity: 1,
            unit_price: 4.99,
          },
        ],
