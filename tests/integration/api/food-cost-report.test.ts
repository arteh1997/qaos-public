...
function setupAuthMock(role = 'Owner') {
  const profileQuery = createChainableMock({
    data: { role, store_id: 'store-1', is_platform_admin: false, default_store_id: null },
    error: null,
  })

  const storeUsersQuery = createChainableMock({
    data: [
      { store_id: 'store-1', user_id: 'user-1', role, store: { id: 'store-1', name: 'Test Store' } },
    ],
    error: null,
  })
...
describe('GET /api/stores/:storeId/reports/food-cost', () => {
  ...
  it('should return 400 when startDate is missing', async () => {
    const req = createRequest('/api/stores/store-1/reports/food-cost?endDate=2026-02-07')
    const res = await GET(req, { params: Promise.resolve({ storeId: 'store-1' }) })
  ...
  it('should return 400 when endDate is missing', async () => {
    const req = createRequest('/api/stores/store-1/reports/food-cost?startDate=2026-02-01')
    const res = await GET(req, { params: Promise.resolve({ storeId: 'store-1' }) })
    expect(res.status).toBe(400)
    const json = await res.json()
  ...
  it('should return 400 for invalid date format', async () => {
    const req = createRequest('/api/stores/store-1/reports/food-cost?startDate=01-02-2026&endDate=07-02-2026')
    const res = await GET(req, { params: Promise.resolve({ storeId: 'store-1' }) })
    expect(res.status).toBe(400)
    const json = await res.json()
  ...
  it('should return 400 when startDate is after endDate', async () => {
    const req = createRequest('/api/stores/store-1/reports/food-cost?startDate=2026-02-10&endDate=2026-02-01')
    const res = await GET(req, { params: Promise.resolve({ storeId: 'store-1' }) })
    expect(res.status).toBe(400)
    const json = await res.json()
  ...
  it('should return food cost report with valid dates', async () => {
    const req = createRequest('/api/stores/store-1/reports/food-cost?startDate=2026-02-01&endDate=2026-02-07')
    const res = await GET(req, { params: Promise.resolve({ storeId: 'store-1' }) })
