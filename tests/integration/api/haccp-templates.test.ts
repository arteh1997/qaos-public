...
describe("HACCP Templates API", () => {
  ...
  const validTemplateBody = {
    ...
    items: [
      {
        id: "1",
        label: "Fridge temperature OK?",
        type: "yes_no",
        required: true,
      },
      {
        id: "2",
        label: "Hand wash station stocked?",
        type: "yes_no",
        required: true,
      },
    ],
  ...
  describe("GET /api/stores/[storeId]/haccp/templates", () => {
    it("should return 401 when not authenticated", async () => {
      ...
      const { GET } =
        await import("@/app/api/stores/[storeId]/haccp/templates/route");
      const request = createMockRequest(
        "GET",
        `/api/stores/${STORE_UUID}/haccp/templates`,
      );
  ...
  describe("GET /api/stores/[storeId]/haccp/templates/[templateId]", () => {
    it("should return 404 for non-existent template", async () => {
      ...
      const { GET } =
        await import("@/app/api/stores/[storeId]/haccp/templates/[templateId]/route");
      const request = createMockRequest(
        "GET",
        `/api/stores/${STORE_UUID}/haccp/templates/${TEMPLATE_UUID}`,
      );
      const response = await GET(request, {
