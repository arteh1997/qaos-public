...
const sampleXeroAccounts = [
  { account_id: 'acc-1', code: '5000', name: 'Cost of Goods Sold', type: 'EXPENSE', class: 'EXPENSE', status: 'ACTIVE' },
  { account_id: 'acc-2', code: '5100', name: 'Produce Costs', type: 'EXPENSE', class: 'EXPENSE', status: 'ACTIVE' },
  { account_id: 'acc-3', code: '5200', name: 'Dairy Costs', type: 'EXPENSE', class: 'EXPENSE', status: 'ACTIVE' },
  { account_id: 'acc-4', code: '1000', name: 'Bank Account', type: 'BANK', class: 'ASSET', status: 'ACTIVE' },
  { account_id: 'acc-5', code: '5300', name: 'Archived Account', type: 'EXPENSE', class: 'EXPENSE', status: 'ARCHIVED' },
]
...
const sampleInvoice = {
  ...
  status: 'applied',
  supplier_id: 'sup-1',
  suppliers: { id: 'sup-1', name: 'Fresh Foods Co', email: 'info@freshfoods.com', phone: '020-1234-5678' },
}
...
describe('Xero Accounting Integration API', () => {
  beforeEach(() => {
    ...
    mockCreateBill.mockResolvedValue({ success: true, external_id: 'xero-bill-001' })
  ...
  describe('GET /api/stores/[storeId]/accounting', () => {
    it('should return connections list without credentials', async () => {
      ...
      const { GET } = await import('@/app/api/stores/[storeId]/accounting/route')
      const request = createMockRequest('GET', `/api/stores/${STORE_UUID}/accounting`)
    ...
    it('should return 403 for unauthorized store access', async () => {
      ...
      const { GET } = await import('@/app/api/stores/[storeId]/accounting/route')
      const request = createMockRequest('GET', `/api/stores/${OTHER_STORE_UUID}/accounting`)
  ...
  describe('GET /api/stores/[storeId]/accounting/accounts', () => {
    it('should return expense accounts from Xero', async () => {
      ...
      const { GET } = await import('@/app/api/stores/[storeId]/accounting/accounts/route')
      const request = createMockRequest('GET', `/api/stores/${STORE_UUID}/accounting/accounts`)
      const response = await GET(request, { params: Promise.resolve({ storeId: STORE_UUID }) })
      const data = await response.json()
      ...
      expect(data.data.accounts.every((a: { class: string }) => a.class === 'EXPENSE')).toBe(true)
    ...
    it('should handle Xero API failure', async () => {
      ...
      mockGetAccounts.mockRejectedValue(new Error('Failed to fetch Xero accounts: 401'))
    ...
    it('should return 403 for unauthorized store', async () => {
      ...
      const { GET } = await import('@/app/api/stores/[storeId]/accounting/accounts/route')
      const request = createMockRequest('GET', `/api/stores/${OTHER_STORE_UUID}/accounting/accounts`)
      const response = await GET(request, { params: Promise.resolve({ storeId: OTHER_STORE_UUID }) })
  ...
  describe('GET /api/stores/[storeId]/accounting/config', () => {
    it('should return GL mappings configuration', async () => {
      ...
      const { GET } = await import('@/app/api/stores/[storeId]/accounting/config/route')
      const request = createMockRequest('GET', `/api/stores/${STORE_UUID}/accounting/config`)
      const response = await GET(request, { params: Promise.resolve({ storeId: STORE_UUID }) })
      const data = await response.json()

      expect(response.status).toBe(200)
      expect(data.success).toBe(true)
      expect(data.data.gl_mappings).toEqual({ Produce: '5100', Dairy: '5200', _default: '5000' })
    })
  ...
  describe('PUT /api/stores/[storeId]/accounting/config', () => {
    it('should update GL mappings successfully', async () => {
      ...
      const { PUT } = await import('@/app/api/stores/[storeId]/accounting/config/route')
      const request = createMockRequest('PUT', `/api/stores/${STORE_UUID}/accounting/config`, {
        gl_mappings: { Produce: '5100', Dairy: '5250', Meat: '5300' },
        auto_sync: true,
      })
      const response = await PUT(request, { params: Promise.resolve({ storeId: STORE_UUID }) })
      const data = await response.json()

      expect(response.status).toBe(200)
      expect(data.success).toBe(true)
      // Merged config should contain the updated values
      expect(data.data.gl_mappings).toEqual({ Produce: '5100', Dairy: '5250', Meat: '5300' })
    ...
    it('should handle database update errors', async () => {
      ...
      const updateErrorQuery = createChainableMock({ data: null, error: { message: 'Update failed' } })
  ...
  describe('POST /api/stores/[storeId]/accounting/sync', () => {
    it('should sync unsynced invoices successfully', async () => {
      ...
      const insertSyncLogQuery = createChainableMock({ data: null, error: null })
      const updateConnectionQuery = createChainableMock({ data: null, error: null })
      ...
      mockAdminClient.from.mockImplementation((table: string) => {
        ...
        if (table === 'accounting_sync_log') {
          ...
          if (existingSyncsQuery.select.mock.calls.length === 0) return existingSyncsQuery
      ...
      const { POST } = await import('@/app/api/stores/[storeId]/accounting/sync/route')
      const request = createMockRequest('POST', `/api/stores/${STORE_UUID}/accounting/sync`, {})
    ...
    it('should record sync failure and report results', async () => {
      ...
      mockCreateBill.mockResolvedValue({ success: false, error: 'Xero API error: 422 Validation error' })
    ...
    it('should return 403 for unauthorized store', async () => {
      ...
      const { POST } = await import('@/app/api/stores/[storeId]/accounting/sync/route')
      const request = createMockRequest('POST', `/api/stores/${OTHER_STORE_UUID}/accounting/sync`, {})
      const response = await POST(request, { params: Promise.resolve({ storeId: OTHER_STORE_UUID }) })
  ...
  describe('POST /api/integrations/xero/disconnect', () => {
    it('should deactivate connection and revoke token', async () => {
      ...
      const request = createMockRequest('POST', '/api/integrations/xero/disconnect', {
        store_id: STORE_UUID,
      })
      ...
      expect(mockRevokeXeroToken).toHaveBeenCalledWith(sampleConnection.credentials)
    })
  ...
  describe('Auth and access control', () => {
    ...
    it('should return 401 when not authenticated (PUT config)', async () => {
      ...
      const { PUT } = await import('@/app/api/stores/[storeId]/accounting/config/route')
      const request = createMockRequest('PUT', `/api/stores/${STORE_UUID}/accounting/config`, { auto_sync: true })
      const response = await PUT(request, { params: Promise.resolve({ storeId: STORE_UUID }) })
    ...
    it('should return 401 when not authenticated (POST disconnect)', async () => {
      ...
      const { POST } = await import('@/app/api/integrations/xero/disconnect/route')
      const request = createMockRequest('POST', '/api/integrations/xero/disconnect', { store_id: STORE_UUID })
      const response = await POST(request)
