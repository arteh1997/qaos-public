...
vi.mock('@/lib/services/pos', () => ({
  ...
  POS_PROVIDERS: {
    square: { name: 'Square', description: 'Square POS', authType: 'oauth2', region: 'Global' },
    toast: { name: 'Toast', description: 'Toast POS', authType: 'oauth2', region: 'US' },
    clover: { name: 'Clover', description: 'Clover POS', authType: 'oauth2', region: 'Global' },
    lightspeed: { name: 'Lightspeed', description: 'Lightspeed POS', authType: 'oauth2', region: 'Global' },
    zettle: { name: 'Zettle', description: 'Zettle by PayPal', authType: 'oauth2', region: 'UK & EU' },
    sumup: { name: 'SumUp', description: 'SumUp POS', authType: 'oauth2', region: 'UK & EU' },
    epos_now: { name: 'Epos Now', description: 'Epos Now cloud POS', authType: 'api_key', region: 'UK' },
    tevalis: { name: 'Tevalis', description: 'Tevalis hospitality', authType: 'api_key', region: 'UK' },
...
function createMockRequest(
  ...
  if (headers) Object.entries(headers).forEach(([k, v]) => reqHeaders.set(k, v))
  return {
    method,
    nextUrl: url,
    url: url.toString(),
...
describe('POS Expansion API', () => {
  ...
  describe('GET /api/integrations/pos/[provider]/auth', () => {
    it('should return redirect for OAuth providers', async () => {
      ...
      const { GET } = await import('@/app/api/integrations/pos/[provider]/auth/route')
      ...
      const response = await GET(request, { params: Promise.resolve({ provider: 'square' }) })
    ...
    it('should return redirect for zettle OAuth provider', async () => {
      ...
      const response = await GET(request, { params: Promise.resolve({ provider: 'zettle' }) })
    ...
    it('should return 400 for API-key providers (epos_now)', async () => {
      ...
      const response = await GET(request, { params: Promise.resolve({ provider: 'epos_now' }) })
    ...
    it('should return 400 for API-key providers (tevalis)', async () => {
      ...
      const response = await GET(request, { params: Promise.resolve({ provider: 'tevalis' }) })
      const data = await response.json()
    ...
    it('should return 400 for unknown provider (null adapter)', async () => {
      ...
      const response = await GET(request, { params: Promise.resolve({ provider: 'unknown_provider' }) })
      const data = await response.json()
  ...
  describe('GET /api/integrations/pos/[provider]/callback', () => {
    it('should redirect with error when error param is present', async () => {
      const { GET } = await import('@/app/api/integrations/pos/[provider]/callback/route')
      const request = createMockRequest(
        'GET',
    ...
    it('should redirect with error for expired state token', async () => {
      ...
      const stateQuery = createChainableMock({ data: expiredState, error: null })
      mockAdminClient.from.mockImplementation(() => stateQuery)
    ...
    it('should redirect to success on valid OAuth callback', async () => {
      ...
      const stateSelectQuery = createChainableMock({ data: validState, error: null })
  ...
  describe('GET /api/stores/[storeId]/pos/menu-items', () => {
    it('should return 400 if connection_id is missing', async () => {
      ...
      const { GET } = await import('@/app/api/stores/[storeId]/pos/menu-items/route')
      const request = createMockRequest(
        'GET',
    ...
    it('should return menu items with mapping status on success', async () => {
      ...
      const mappingsQuery = createChainableMock({
        data: [
          { pos_item_id: 'sq-item-1', inventory_item_id: 'inv-1', is_active: true },
        ],
        error: null,
      })
      ...
      mockGetAdapter.mockReturnValue({
        ...
        fetchMenuItems: vi.fn().mockResolvedValue([
          { pos_item_id: 'sq-item-1', pos_item_name: 'Cheeseburger', price: 12.99, category: 'Mains' },
          { pos_item_id: 'sq-item-2', pos_item_name: 'Fries', price: 4.99, category: 'Sides' },
        ]),
        validateSignature: vi.fn(),
        normalizeEvent: vi.fn(),
      })
      ...
      expect(data.data[0].mapping).toEqual({ pos_item_id: 'sq-item-1', inventory_item_id: 'inv-1', is_active: true })
  ...
  describe('POST /api/pos/webhook/[connectionId]', () => {
    it('should return 404 for invalid connection', async () => {
      ...
      const request = createMockRequest(
        ...
        { event_id: 'evt-1', items: [{ pos_item_id: 'i1', pos_item_name: 'Item', quantity: 1 }] }
      )
      const response = await POST(request, { params: Promise.resolve({ connectionId: 'nonexistent-id' }) })
      const data = await response.json()
    ...
    it('should return 403 for inactive connection', async () => {
      ...
      const response = await POST(request, { params: Promise.resolve({ connectionId: CONNECTION_UUID }) })
      const data = await response.json()
    ...
    it('should return 401 for invalid webhook signature when secret is configured', async () => {
      ...
      const { POST } = await import('@/app/api/pos/webhook/[connectionId]/route')
      const body = { event_id: 'evt-1', items: [{ pos_item_id: 'i1', pos_item_name: 'Item', quantity: 1 }] }
      const request = createMockRequest(
        'POST',
        `/api/pos/webhook/${CONNECTION_UUID}`,
        body,
    ...
    it('should return 400 for missing event_id in generic format', async () => {
      ...
      const { POST } = await import('@/app/api/pos/webhook/[connectionId]/route')
      const body = { items: [{ pos_item_id: 'i1', pos_item_name: 'Item', quantity: 1 }] }
      // Missing event_id
      const request = createMockRequest(
        'POST',
        `/api/pos/webhook/${CONNECTION_UUID}`,
    ...
    it('should return 201 for valid sale event (processed)', async () => {
      ...
      const body = {
        ...
        event_type: 'sale',
        items: [
          { pos_item_id: 'burger-1', pos_item_name: 'Cheeseburger', quantity: 2, unit_price: 12.99 },
          { pos_item_id: 'fries-1', pos_item_name: 'Fries', quantity: 1, unit_price: 4.99 },
        ],
        total_amount: 30.97,
        currency: 'GBP',
    ...
    it('should use provider-specific signature header', async () => {
      ...
      mockGetAdapter.mockReturnValue({
        ...
        normalizeEvent: vi.fn().mockReturnValue({
          ...
          event_type: 'sale',
          items: [{ pos_item_id: 'sq-i1', pos_item_name: 'Pizza', quantity: 1 }],
    ...
    it('should handle JSON parse errors gracefully', async () => {
      ...
      const url = new URL(`http://localhost:3000/api/pos/webhook/${CONNECTION_UUID}`)
      const request = {
        method: 'POST',
        nextUrl: url,
        url: url.toString(),
