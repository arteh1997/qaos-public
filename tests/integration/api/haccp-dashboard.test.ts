...
function createMockRequest(): NextRequest {
  const url = new URL(
    `http://localhost:3000/api/stores/${STORE_UUID}/haccp/dashboard`,
  );
...
describe("HACCP Dashboard API Tests", () => {
  ...
  describe("GET /api/stores/:storeId/haccp/dashboard", () => {
    it("should return 401 when not authenticated", async () => {
      ...
      const { GET } =
        await import("@/app/api/stores/[storeId]/haccp/dashboard/route");
    ...
    it("should return dashboard data with compliance score", async () => {
        ...
        data: [
          {
            ...
            status: "fail",
            completed_at: "2026-02-25T11:00:00Z",
            haccp_check_templates: { name: "Opening Check" },
          },
          {
            id: "check-2",
            status: "pass",
            completed_at: "2026-02-25T10:00:00Z",
            haccp_check_templates: { name: "Temp Check" },
          },
          {
            id: "check-1",
            status: "pass",
            completed_at: "2026-02-25T09:00:00Z",
            haccp_check_templates: { name: "Delivery Check" },
          },
        ],
        error: null,
      });
      ...
      const recentTempAlertsQuery = createChainableMock({
        data: [
          {
            id: "temp-1",
            equipment_name: "Walk-in Fridge",
            temperature: 8.5,
            min_temp: 0,
            max_temp: 5,
            recorded_at: "2026-02-25T08:30:00Z",
            is_in_range: false,
          },
        ],
        error: null,
      ...
      mockSupabaseClient.from.mockImplementation((table: string) => {
        ...
        if (table === "haccp_temperature_logs") {
          ...
          return haccpTempLogsCallCount === 1
            ? outOfRangeTempsQuery
            : recentTempAlertsQuery;
        }
    ...
    it("should include unresolved corrective actions count", async () => {
      ...
      const unresolvedActionsQuery = createChainableMock({
        data: [{ id: "action-1" }, { id: "action-2" }, { id: "action-3" }],
        error: null,
      });
        ...
        data: [
          {
            ...
            status: "pass",
            completed_at: "2026-02-25T12:00:00Z",
            haccp_check_templates: { name: "Closing Check" },
          },
        ],
        error: null,
      });
