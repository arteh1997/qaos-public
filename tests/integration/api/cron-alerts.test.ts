...
vi.mock('@/lib/services/alertService', () => ({
  processScheduledAlerts: (...args: unknown[]) => mockProcessScheduledAlerts(...args),
}))
...
describe('Cron Alerts API', () => {
  ...
  describe('POST /api/cron/send-alerts', () => {
    ...
    it('should process alerts with valid secret', async () => {
      ...
      const { POST } = await import('@/app/api/cron/send-alerts/route')
      const request = createMockRequest({ authorization: 'Bearer test-cron-secret' })
      const response = await POST(request)
    ...
    it('should report mixed results', async () => {
      mockProcessScheduledAlerts.mockResolvedValue([
        { store_id: 's1', store_name: 'Store 1', alert_type: 'low_stock', success: true },
        { store_id: 's2', store_name: 'Store 2', alert_type: 'critical_stock', success: false, error: 'Email failed' },
      ])
