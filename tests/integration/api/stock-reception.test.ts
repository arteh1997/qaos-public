...
function createMockRequest(
  ...
  const url = new URL(
    `http://localhost:3000/api/stores/${storeId}/stock-reception`,
  );
  ...
  describe("POST /api/stores/:storeId/stock-reception", () => {
    describe("Authentication", () => {
      it("should return 401 when not authenticated", async () => {
        ...
        const { POST } =
          await import("@/app/api/stores/[storeId]/stock-reception/route");
    ...
    describe("Authorization", () => {
      it("should allow Owner users", async () => {
        ...
        currentInventoryQuery.upsert = vi
          .fn()
    ...
    describe("Access Revocation Check", () => {
      it("should return 403 if access was revoked mid-operation", async () => {
        ...
        const { profileQuery } = setupAuthenticatedUser(
          "Owner",
          OTHER_STORE_UUID,
        );
    ...
    describe("Successful Reception", () => {
      ...
      it("should update unit_cost when total_cost is provided", async () => {
        ...
        const request = createMockRequest({
          items: [
            { inventory_item_id: ITEM_UUID_1, quantity: 50, total_cost: 20 },
          ],
        });
        const response = await POST(request, {
          params: Promise.resolve({ storeId: STORE_UUID }),
