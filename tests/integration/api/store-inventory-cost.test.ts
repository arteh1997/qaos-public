...
function createMockRequest(body?: object): NextRequest {
  const url = new URL(
    "http://localhost:3000/api/stores/store-1/inventory/item-123",
  );
...
describe("PATCH /api/stores/:storeId/inventory/:itemId - Cost Update", () => {
  beforeEach(() => {
    ...
    mockAdminClient.from.mockReturnValue(
      createChainableMock({ data: { name: "Test Item" }, error: null }),
    );
  });
  ...
  it("should update cost when store_inventory record exists", async () => {
    ...
    const { PATCH } =
      await import("@/app/api/stores/[storeId]/inventory/[itemId]/route");

    const request = createMockRequest({ unit_cost: 3.5 });
    ...
    expect(data.data.unit_cost).toBe(3.5);
  ...
  it("should create store_inventory record when none exists (upsert)", async () => {
    ...
    const request = createMockRequest({ unit_cost: 5.5 });
    const response = await PATCH(request, {
      params: Promise.resolve({ storeId: "store-1", itemId: "item-123" }),
    });
    const data = await response.json();
    ...
    expect(response.status).toBe(200);
    expect(data.success).toBe(true);
    expect(data.data.unit_cost).toBe(5.5);
  ...
  it("should return 403 for Staff users", async () => {
    ...
    const request = createMockRequest({ unit_cost: 5.0 });
    const response = await PATCH(request, {
      params: Promise.resolve({ storeId: "store-1", itemId: "item-123" }),
    });
    const data = await response.json();
  ...
  it("should update both cost and par_level together", async () => {
    ...
    const request = createMockRequest({ unit_cost: 4.0, par_level: 15 });
    const response = await PATCH(request, {
      params: Promise.resolve({ storeId: "store-1", itemId: "item-123" }),
    });
    const data = await response.json();
    ...
    expect(response.status).toBe(200);
    expect(data.success).toBe(true);
    expect(data.data.unit_cost).toBe(4.0);
...
describe("DELETE /api/stores/:storeId/inventory/:itemId - Delete Item", () => {
  ...
  it("should clean up operational data and soft-delete item, preserving history", async () => {
    ...
    const touchedTables = mockAdminClient.from.mock.calls.map(
      (c: string[]) => c[0],
    );
