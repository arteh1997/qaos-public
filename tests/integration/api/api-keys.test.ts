...
describe('API Keys Management', () => {
  ...
  describe('GET /api/stores/[storeId]/api-keys', () => {
    it('should return 401 when not authenticated', async () => {
      ...
      const request = createMockRequest('GET', `/api/stores/${STORE_UUID}/api-keys`)
    ...
    it('should return API keys for Owner', async () => {
      ...
      const keysQuery = createChainableMock({
        data: [
          { id: 'key-1', name: 'Production', key_prefix: 'rk_live_a1b2', scopes: ['*'], is_active: true, last_used_at: null, expires_at: null, created_at: '2026-02-10' },
        ],
        error: null,
      })
  ...
  describe('POST /api/stores/[storeId]/api-keys', () => {
    it('should create an API key for Owner', async () => {
      ...
      const insertQuery = createChainableMock({
        data: { id: 'key-2', name: 'Test Key', key_prefix: 'rk_live_test', scopes: ['inventory:read'], expires_at: null, created_at: '2026-02-10' },
        error: null,
      })
      ...
      const { POST } = await import('@/app/api/stores/[storeId]/api-keys/route')
      const request = createMockRequest('POST', `/api/stores/${STORE_UUID}/api-keys`, {
        name: 'Test Key',
  ...
  describe('DELETE /api/stores/[storeId]/api-keys', () => {
    it('should revoke an API key', async () => {
      ...
      const { DELETE } = await import('@/app/api/stores/[storeId]/api-keys/route')
      const request = createMockRequest('DELETE', `/api/stores/${STORE_UUID}/api-keys`, undefined, { keyId: 'key-1' })
    ...
    it('should return 400 without keyId', async () => {
      ...
      const { DELETE } = await import('@/app/api/stores/[storeId]/api-keys/route')
      const request = createMockRequest('DELETE', `/api/stores/${STORE_UUID}/api-keys`)
      const response = await DELETE(request, { params: Promise.resolve({ storeId: STORE_UUID }) })
      expect(response.status).toBe(400)
    })
  })
