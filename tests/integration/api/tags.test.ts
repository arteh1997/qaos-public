...
describe('Tags API', () => {
  ...
  describe('GET /api/stores/[storeId]/tags', () => {
    it('should return tags with usage counts for authorized user', async () => {
      ...
      const mockTags = [
        { id: 'tag-1', name: 'Perishable', description: 'Items that expire quickly', color: '#EF4444', created_at: '2024-01-01' },
        { id: 'tag-2', name: 'High-Value', description: 'Expensive items', color: '#F59E0B', created_at: '2024-01-01' },
      ]
  ...
  describe('POST /api/stores/[storeId]/tags', () => {
    it('should create a new tag for Owner/Manager', async () => {
      ...
      mockSupabaseClient.from.mockImplementation((table: string) => {
        ...
        if (table === 'item_tags') {
          ...
          mockChain.insert = vi.fn(() => createChainableMock({ data: { id: 'tag-new', ...newTag }, error: null }))
          return mockChain
        }
        return createChainableMock()
      })
    ...
    it('should reject duplicate tag name', async () => {
      ...
      mockSupabaseClient.from.mockImplementation((table: string) => {
        ...
        if (table === 'store_users') return storeUsersQuery
        if (table === 'item_tags') {
          return createChainableMock({ data: { id: 'existing-tag' }, error: null })
        }
        return createChainableMock()
      })
  ...
  describe('PATCH /api/stores/[storeId]/tags/[tagId]', () => {
    it('should update tag for Owner/Manager', async () => {
      ...
      const { PATCH } = await import('@/app/api/stores/[storeId]/tags/[tagId]/route')
  ...
  describe('DELETE /api/stores/[storeId]/tags/[tagId]', () => {
    it('should delete tag for Owner only', async () => {
      ...
      mockSupabaseClient.from.mockImplementation((table: string) => {
        ...
        if (table === 'item_tags') {
          const mockChain = createChainableMock()
          mockChain.select = vi.fn(() => createChainableMock({ data: { id: mockTagId }, error: null }))
      ...
      const { DELETE } = await import('@/app/api/stores/[storeId]/tags/[tagId]/route')
