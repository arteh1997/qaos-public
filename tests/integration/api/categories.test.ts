...
describe('Categories API', () => {
  ...
  function setupAuthMocks(role: string = 'Manager') {
    ...
    const storeUsersData = [{ id: 'su-1', store_id: mockStoreId, user_id: mockUserId, role }]
  ...
  describe('DIAGNOSTIC', () => {
    it('should work with stores route (sanity check)', async () => {
      ...
      const storeUsersQuery = createChainableMock({
        data: [{ id: 'su-1', store_id: mockStoreId, user_id: mockUserId, role: 'Owner' }],
        error: null,
      })
  ...
  describe('GET /api/stores/[storeId]/categories', () => {
    it('should return categories with item counts for authorized user', async () => {
      ...
      const mockCategories = [
        { id: 'cat-1', name: 'Produce', description: 'Fresh produce', color: '#22C55E', sort_order: 1, created_at: '2024-01-01', updated_at: '2024-01-01' },
        { id: 'cat-2', name: 'Dairy', description: 'Dairy products', color: '#3B82F6', sort_order: 2, created_at: '2024-01-01', updated_at: '2024-01-01' },
      ]
      ...
      const { GET } = await import('@/app/api/stores/[storeId]/categories/route')
      const request = createMockRequest('GET')
    ...
    it('should return 403 for unauthorized user', async () => {
      ...
      mockSupabaseClient.from.mockImplementation((table: string) => {
        ...
        if (table === 'store_users') {
          ...
          noAccessQuery.single = vi.fn().mockResolvedValue({ data: null, error: null })
    ...
    it('should create a new category for Owner/Manager', async () => {
      ...
      mockSupabaseClient.from.mockImplementation((table: string) => {
        ...
        if (table === 'item_categories') {
          ...
          mockChain.select = vi.fn(() => createChainableMock({ data: null, error: null }))
          // Mock insert
          mockChain.insert = vi.fn(() => createChainableMock({ data: { id: 'cat-new', ...newCategory }, error: null }))
          return mockChain
        }
        return createChainableMock()
      })

      const { POST } = await import('@/app/api/stores/[storeId]/categories/route')
      const request = createMockRequest('POST', newCategory)
      const response = await POST(request, { params: Promise.resolve({ storeId: mockStoreId }) })
      const data = await response.json()
    ...
    it('should reject duplicate category name', async () => {
      ...
      mockSupabaseClient.from.mockImplementation((table: string) => {
        ...
        if (table === 'item_categories') {
          ...
          return createChainableMock({ data: { id: 'existing-cat' }, error: null })
        }
        return createChainableMock()
      })
  ...
  describe('PATCH /api/stores/[storeId]/categories/[categoryId]', () => {
    it('should update category for Owner/Manager', async () => {
      ...
      mockSupabaseClient.from.mockImplementation((table: string) => {
        ...
        if (table === 'item_categories') {
          ...
          mockChain.single = vi.fn().mockImplementation(() => {
            ...
            return Promise.resolve({ data: { id: mockCategoryId }, error: null })
          })
      ...
      const { PATCH } = await import('@/app/api/stores/[storeId]/categories/[categoryId]/route')
      const request = createMockRequest('PATCH', updates)
      const response = await PATCH(request, {
        params: Promise.resolve({ storeId: mockStoreId, categoryId: mockCategoryId })
      })
      const data = await response.json()
  ...
  describe('DELETE /api/stores/[storeId]/categories/[categoryId]', () => {
    it('should delete category for Owner only', async () => {
      ...
      mockSupabaseClient.from.mockImplementation((table: string) => {
        ...
        if (table === 'item_categories') {
          const mockChain = createChainableMock()
          mockChain.select = vi.fn(() => createChainableMock({ data: { id: mockCategoryId }, error: null }))
          mockChain.delete = vi.fn(() => createChainableMock({ data: null, error: null }))
          return mockChain
        }
        if (table === 'inventory_items') {
          return createChainableMock({ data: [], error: null })
      ...
      const { DELETE } = await import('@/app/api/stores/[storeId]/categories/[categoryId]/route')
    ...
    it('should prevent deletion if items are using the category', async () => {
      ...
      mockSupabaseClient.from.mockImplementation((table: string) => {
        ...
        if (table === 'store_users') return storeUsersQuery
        if (table === 'item_categories') {
          return createChainableMock({ data: { id: mockCategoryId }, error: null })
        }
        if (table === 'inventory_items') {
