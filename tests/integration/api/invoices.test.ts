...
describe("Invoices API", () => {
  ...
  describe("GET /api/stores/[storeId]/invoices", () => {
    it("should return invoices list with pagination for Owner", async () => {
      ...
      const request = createMockRequest(
        "GET",
        `/api/stores/${STORE_UUID}/invoices`,
      );
  ...
  describe("POST /api/stores/[storeId]/invoices", () => {
    it("should upload an invoice file for Owner", async () => {
      ...
      const file = new File(["fake pdf content"], "invoice.pdf", {
      ...
      const { POST } =
        await import("@/app/api/stores/[storeId]/invoices/route");
    ...
    it("should return 400 for oversized file", async () => {
      ...
      const file = new File([bigContent], "big-invoice.pdf", {
        type: "application/pdf",
      });
      formData.append("file", file);
    ...
    it("should handle storage upload failure", async () => {
      ...
      mockAdminClient.storage.from.mockReturnValue({
        upload: vi
          .fn()
          .mockResolvedValue({ error: { message: "Storage quota exceeded" } }),
        remove: vi.fn().mockResolvedValue({ error: null }),
      });
  ...
  describe("GET /api/stores/[storeId]/invoices/[invoiceId]", () => {
    it("should return invoice with line items and signed URL", async () => {
      ...
      mockAdminClient.storage.from.mockReturnValue({
        createSignedUrl: vi.fn().mockResolvedValue({
          data: {
            signedUrl:
              "https://storage.example.com/signed/invoice.pdf?token=abc",
          },
        }),
      });

      const { GET } =
        await import("@/app/api/stores/[storeId]/invoices/[invoiceId]/route");
      const request = createMockRequest(
        "GET",
        `/api/stores/${STORE_UUID}/invoices/${INVOICE_UUID}`,
      );
      const response = await GET(request, {
      ...
      expect(data.data.file_url).toBe(
        "https://storage.example.com/signed/invoice.pdf?token=abc",
      );
    });
    ...
    it("should return 404 when invoice not found", async () => {
      ...
      const request = createMockRequest(
        "GET",
        `/api/stores/${STORE_UUID}/invoices/nonexistent-id`,
      );
      const response = await GET(request, {
        params: Promise.resolve({
          storeId: STORE_UUID,
  ...
  describe("PATCH /api/stores/[storeId]/invoices/[invoiceId]", () => {
    ...
    it("should update line items along with invoice fields", async () => {
      ...
      const updateQuery = createChainableMock({ data: null, error: null });
      const lineItemUpdateQuery = createChainableMock({
        data: null,
        error: null,
      });
      const lineItemInsertQuery = createChainableMock({
        data: null,
        error: null,
      });
      const updatedQuery = createChainableMock({
        ...
        `/api/stores/${STORE_UUID}/invoices/${INVOICE_UUID}`,
        {
          ...
          line_items: [
            ...
            },
            {
              ...
              unit_price: 10,
              total_price: 50,
              inventory_item_id: "55555555-5555-4555-a555-555555555555",
            },
          ],
        },
      );
  ...
  describe("POST /api/stores/[storeId]/invoices/[invoiceId]/apply", () => {
    it("should apply an approved invoice to inventory", async () => {
      ...
      const { POST } =
        await import("@/app/api/stores/[storeId]/invoices/[invoiceId]/apply/route");
      const request = createMockRequest(
        "POST",
        `/api/stores/${STORE_UUID}/invoices/${INVOICE_UUID}/apply`,
        {
  ...
  describe("Auth and access control", () => {
    ...
    it("should return 403 when user does not belong to the store (GET list)", async () => {
      ...
      const request = createMockRequest(
        "GET",
        `/api/stores/${OTHER_STORE_UUID}/invoices`,
      );
      const response = await GET(request, {
    ...
    it("should return 403 when user does not belong to the store (POST apply)", async () => {
      ...
      const request = createMockRequest(
        "POST",
        `/api/stores/${OTHER_STORE_UUID}/invoices/${INVOICE_UUID}/apply`,
      );
      const response = await POST(request, {
        params: Promise.resolve({
          storeId: OTHER_STORE_UUID,
          invoiceId: INVOICE_UUID,
        }),
      });
