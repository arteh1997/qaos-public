...
describe('Shift Detail API Tests', () => {
  ...
  describe('GET /api/shifts/:shiftId', () => {
    it('should return 401 when not authenticated', async () => {
      ...
      const response = await GET(request, { params: Promise.resolve({ shiftId: 'shift-123' }) })
      ...
      const shiftQuery = {
        ...
        single: vi.fn().mockResolvedValue({
          data: {
            ...
            user: { id: 'user-456', full_name: 'John Doe', email: 'john@example.com' },
          },
          error: null,
        }),
      }
    ...
    it('should return 404 when shift not found', async () => {
      ...
      const request = createMockRequest('GET')
      const response = await GET(request, { params: Promise.resolve({ shiftId: 'nonexistent' }) })
      const data = await response.json()
  ...
  describe('PATCH /api/shifts/:shiftId', () => {
    it('should return 401 when not authenticated', async () => {
      ...
      const response = await PATCH(request, { params: Promise.resolve({ shiftId: 'shift-123' }) })
      const data = await response.json()
  ...
  describe('DELETE /api/shifts/:shiftId', () => {
    it('should return 401 when not authenticated', async () => {
      ...
      const response = await DELETE(request, { params: Promise.resolve({ shiftId: 'shift-123' }) })
      const data = await response.json()
